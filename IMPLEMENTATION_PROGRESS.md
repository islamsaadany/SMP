...
**2026-09-24 — the four checks that were red on main (§410.2), on the branch.** Three were the checks being out of date, and are rewritten so they still catch real faults (each proved by breaking the product on purpose): the Merchandising Overview now has only an "Add an objective" button when it holds no objectives, the Figure sets mark is drawn by a fallback font and only looked missing to a width test, and a new figure's unit is written without a space. The fourth is a real fault waiting on Islam: in the register's edit dialog the grey Sign-in name box is 10px taller than the boxes beside it, which makes the dialog scroll on a 1280×700 window.
