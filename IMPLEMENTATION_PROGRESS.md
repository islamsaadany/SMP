...
**2026-09-24 — the design standard (§411), drawn, waiting on Islam.** A one-page standard every page will be built from: six text sizes, four spacing steps, three corner roundings, two shadows, one card, one drawn icon set, colours by name only, and Source Sans for everything including headings. Each piece is shown beside how it looks today. Nothing in the product has changed yet; pages move to it one at a time, each with its own mockup. Waiting on Islam: sign-off on the standard, and one choice inside it (codes like MB01 in Source Sans rather than the typewriter face).
