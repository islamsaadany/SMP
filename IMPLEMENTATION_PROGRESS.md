...
**2026-09-25 — back into a module from the trail's last menu (§411), on the branch, not merged.** On Client settings the last menu in the top trail only offered each module's settings, and on a module's settings it left out that module, so there was no way back into a module from it. Every such menu now starts with the modules themselves (Strategy, Insights), then a line, then the settings pages. On a module's normal page it is unchanged.
