...
**2026-09-24 — the standard in the stylesheet, and the Plan page drawn (§412), on the branch.** The standard's sizes, spacing, corners and shadows are now in the stylesheet beside today's; nothing on screen moved (checked by comparing screenshots, identical). A new check counts every size, corner, shadow and colour typed in directly instead of taken from the standard, and fails if any count goes up, so the mess can only shrink. The Plan page on the standard is drawn beside today's, reading and editing, light and dark. Waiting on Islam: sign-off on the Plan page look, and whether the outcome column goes from bold to regular text.

**2026-09-24 — the design standard (§411), signed off.** A one-page standard every page will be built from: six text sizes, four spacing steps, three corner roundings, two shadows, one card, one drawn icon set, colours by name only, and Source Sans for everything including headings and codes like MB01. The navigation stays as three rows across the top. Nothing in the product has changed yet. Next: add the standard's sizes to the stylesheet (nothing visible moves), then move pages one at a time, each with its own mockup.
