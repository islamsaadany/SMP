...
| `../../design-mockups/setup-refinements/2026-08-27_setup-header-line.html` | Every Setup page's search, filters and buttons share the pinned line with its name; the `SMO` pill, the count chips and the briefing paragraph go; Send a message becomes Send an email with the Email settings folded in. Not a drawing &mdash; the real platform driven in a browser with the proposal injected into the live page, plus two options for the company field. **Built in 3.48** (&sect;130) |
| `../../design-mockups/setup-refinements/2026-08-27_marking-and-the-matrix.html` | Focus measures as a switch on the header line, a navigation-style destination row and one table headed like the register; and the diagnosed repair for the Roles &amp; access header, which was pinning 141px down inside its own scroll box. Before and after on the same build. **Built in 3.48** (&sect;130.2, &sect;130.5) |
