...
/* §288: AND A HIDDEN TACTIC IS NOT ONE OF THE THREE. `missWord` answers for
        ...
        : miniTable(["#","Key objective","Direction","Target","Actual","Progress"],
            c.keyObjectives.map(function(m, i){
              ...
              return '<tr><td class="idx">' + (i+1) + '</td><td>' + esc(m.name) + '</td>' +
                ...
                '<td class="num">' + (m.target ? tgtShown(m.target) : missWord(m)) + '</td>' +
...
function renderTemple(){
  ...
  var cell = function(m){
    return '<div class="ns-item"><span class="ns-label">' + esc(m.name) + '</span>' +
           '<span class="ns-target">' + (m.target ? tgtShown(m.target) : missWord(m)) + '</span>' +
  ...
  return '<div class="ohead' + (near ? '' : ' one') + '"><span>Objective</span>' +
    ...
    list.map(function(m){
      return '<div class="orow' + (near ? '' : ' one') +
        ...
        /* §288: the word is asked OF THE ROW, so a hidden objective — which
           the counts stopped counting at §233 — reads a dash beside its own
           "Hidden — not counted" chip rather than arguing with it. main's
           `tgtShown` keeps the value's own reading (§243's scale); only the
           BLANK changes hands. */
        '<span class="ot h">' + (m.target3y ? tgtShown(m.target3y) : missWord(m)) + '</span>' +
        (near ? '<span class="ot">' + (m.target ? tgtShown(m.target) : missWord(m)) + '</span>' : '') + '</div>';
    }).join("");
}
...
/* ── THE WORD A BLANK WEARS, AND A HIDDEN ROW DOES NOT WEAR IT (§288) ──
  ...
  var objTable = miniTable(["#", L("keyobj","bu"), "Dir.", "Target", "Reported", "Note"],
    objs.map(function(x, i){
      return '<tr' + (needsNote(x) ? ' class="wantnote"' : '') + '>' +
        ...
        '<td class="num">' + (x.obj.target ? tgtShown(x.obj.target) : missWord(x.obj)) + '</td>' +
...
/* §288: `row` is optional and is only ever the milestone the date belongs
...
function unitPlanBody(it, u, railed){
  ...
  /* §288: the local `cell` helper is GONE, not left standing — §249 moved
  ...
  var tRows = it.tactics.map(function(t, i){
    ...
    return '<tr data-oi="' + i + '"' + hidCls(t) + '><td class="idx">' +
      ...
      '<td>' + gapCell("plan", "u_plan", t, "owner", {
        ctx:pctx(t),
        /* §288: asked of the row, so a hidden tactic's empty owner reads a
