...
function deckSlides(u){
  ...
  /* \u2500\u2500 A SUPPORTING FUNCTION AIMS AT ITS OBJECTIVES, AND NOTHING ELSE (\u00a7237)
     ...
     He is right on every count and the data says so. A supporting function
     INHERITS its aspiration and its SWOT from the unit it plans under and
     never authors either (\u00a7213), so the label was standing over an empty
     paragraph; and its objectives carry a WEIGHT and no 3-year target (\u00a7213
     again), so `Dir.` and the horizon column were two columns of em-dashes.

     THE THIS-YEAR COLUMN IS UNCONDITIONAL HERE, and that is not a detail:
     `SHOW_KO_THIS_YEAR` is a per-viewer setting (\u00a766), so on a function \u2014
     whose only target this is \u2014 a viewer who had turned it off would get a
     table of objectives with no target at all. Islam settled the reason
     himself: *"the functions has no 3 years objectives."*

     A BUSINESS UNIT'S SLIDE IS UNTOUCHED and it is asserted, because a unit
     does author an aspiration and does carry a 3-year target. */
  var fnAim = !!u.fnKey;
  ...
  var aimRows = SMPRules.shown(u.keyObjectives).map(function(m, i){
    return '<tr><td class="idx">' + (i+1) + '</td>' +
      ...
      (fnAim ? '' : '<td class="num">' + esc(m.dir) + '</td>' +
  ...
  S.push('<section class="dslide"' + anch("aim", "After \u201cWhat we are aiming at\u201d") +
    ...
    '<div class="aimbottom">' +
      ...
      (fnAim ? '' : '<th class="num">Dir.</th><th class="num">' + horizonColLabel() + '</th>') +
