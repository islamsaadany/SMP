...
SELECT d.key, d.idx, d.internal, d.one,
       CASE WHEN d.key = 'capability'
             AND (SELECT grp FROM labels WHERE key = 'pillar') IS NOT NULL
             AND (SELECT grp FROM labels WHERE key = 'pillar') NOT IN ('', '—', 'Group capabilities', 'Pillar')
            THEN (SELECT grp FROM labels WHERE key = 'pillar') ELSE d.many END,
       d.note
  FROM d
