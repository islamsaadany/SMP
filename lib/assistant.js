...
/* THE TENANT'S OWN WORDS OVER THE SHIPPED ONES (§140). The precedence is
   `Rules.kbLook` — the same rule the page renders by — so the assistant can
   never answer from a wording the page has stopped showing. Additions join
   the corpus as ordinary recipes; `who` is "everyone", because the office
   wrote them for the people who ask. */
function withTenant(kb, t) {
  if (!kb || !t) return kb;
  const recs = (kb.recipes || []).map(function (r) {
    const o = Rules.kbLook(t, r.id);
    /* THE AUDIENCE OVERRIDE RIDES WITH THE WORDING (§161). It lives in the
       same `ov` entry, so a tenant that has only retagged a question and
       not reworded it still comes back through kbLook — which is why that
       function tests `e.w` as well as `e.q`/`e.a`. */
    return o ? Object.assign({}, r, { q: o.q || r.q, a: o.a || r.a,
                                      who: Rules.kbAudienceWord(o.w || r.who) }) : r;
  });
  const extra = Rules.kbAllAdds(t).map(function (x) {
    return { id: x.id, group: x.g || "", q: x.q || "", a: x.a || "",
             who: Rules.kbAudienceWord(x.w) };
  });
  return Object.assign({}, kb, { recipes: recs.concat(extra) });
}

/* ── AN OFFICE ANSWER IS WITHHELD, NOT MERELY MARKED (2026-08-29) ─────
   `who` used to be RELEVANCE and never permission: the whole corpus went to
   the model and the tag only chose between two public answers. Islam has
   settled that the Strategy Office's operational answers are the office's —
   so a non-office asker never sees them, and the question falls through to
   `answered=false` and reaches a person, which is the right destination for
   somebody asking about something only the office can do.

   THE TAGS HAD TO BE RE-READ BEFORE THIS COULD BE TURNED ON, and three of the
   eight were wrong — a tag written to mean "only useful to the office" is a
   bad tag to start enforcing unread. Presenting a review has NO gate at all,
   Manage slides is `canSpeakFor` (a unit's head or custodian), and the Demo
   data menu is not role-gated in any way. All three are answers now, tagged
   for everyone and moved to the sections whose readers can act on them.

   §161 MADE IT THREE, at Islam's direction: `office` (the office alone),
   `others` (everybody but the office) and `everyone` (both, and the default
   an absent value reads as). The third is what makes a two-answer pair
   DETERMINISTIC — §160 handed the office both halves with an instruction to
   choose, and now each side is served exactly one. `Rules.kbSees()` is the
   one place that answers it, so the client, the server and the checks cannot
   drift. */
function officeOnly(kb, isOffice) {
  return Object.assign({}, kb, {
    recipes: (kb.recipes || []).filter(function (r) {
      return Rules.kbSees(r.who, isOffice);
    })
  });
}
...
function corpusText(kb, labels) {
  ...
  (kb.recipes || []).forEach(function (r) {
    parts.push("## [" + r.id + "] " + sub(r.q) +
               (r.who === "office" ? "\n(written for the Strategy Office)" : "") +
               (r.who === "others" ? "\n(written for somebody outside the Strategy Office)" : "") +
               "\n" + Rules.kbParas(sub(r.a)).join("\n\n"));
  });
  return parts.join("\n\n");
}
...
async function ask(opts) {
  ...
  if (!res.ok) {
    ...
    /* THE KNOB, NOT THE QUESTION (§134.5, corrected from production the day
       it shipped). The first version retried only when the 400 NAMED thinking
       — because the stub imitated Google's verbose "Unknown name" refusal —
       and the real provider answered gemini-3.6-flash's refusal with the
       generic "Request contains an invalid argument." and nothing else. The
       guard never fired and the knob took the assistant down, which is the
       precise fault it exists to prevent. §100.3's rule cuts deeper than it
       read: a stub models the PROVIDER, and I had modelled the documentation.

       So the signal is the SITUATION, never the wording: any 400 on a request
       that carried the knob, once per process, after the bad-key check (a bad
       key's 400 must never be retried into). Worst case is one extra request
