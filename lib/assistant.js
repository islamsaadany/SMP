...
async function ask(opts) {
  ...
  const body = {
    ...
    generationConfig: {
      ...
      temperature: 0,
      /* THINKING COUNTS (§132). On Gemini 2.5+ the model reasons before it
         answers and the reasoning is billed against maxOutputTokens — so a
         700 cap could be eaten whole by thought, leaving a truncated or empty
         JSON that parses as a failure and, by §112.2's design, writes nothing
         at all. The schema keeps the visible reply short; the headroom is for
         what the model spends before it starts speaking. */
