...
function withTenant(kb, t) {
  ...
  const recs = (kb.recipes || []).map(function (r) {
    ...
    return o ? Object.assign({}, r, { q: o.q || r.q, a: o.a || r.a }) : r;
  });
  const extra = Rules.kbAllAdds(t).map(function (x) {
    return { id: x.id, group: x.g || "", q: x.q || "", a: x.a || "", who: "everyone" };
  });
...
/* ── AN OFFICE ANSWER IS WITHHELD, NOT MERELY MARKED (2026-08-29) ─────
   ...
   THE PAIR STILL WORKS, and is worth more than it was: a question with two
   true answers keeps both, and each side now sees only the one written for
   it rather than both. */
function officeOnly(kb, isOffice) {
  if (isOffice) return kb;
  return Object.assign({}, kb, {
    recipes: (kb.recipes || []).filter(function (r) { return r.who !== "office"; })
  });
}
