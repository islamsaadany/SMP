...
/* WHAT A HANDOFF SAYS (§119), and it lives HERE rather than beside the INSERT
...
/* TRIMMED, AND QUOTES STRIPPED (§117). A key pasted into a dashboard field
...
/* PRESENT, NOT VALID. Everything this can see is that a value exists — whether
   the provider accepts it is a question only the provider answers, and §117
...
/* WHAT KEY IS THIS, WITHOUT SAYING WHAT KEY THIS IS (§120). "The key is
   ...
   Its LENGTH and its first four characters are enough and are not secret: a
   Google AI Studio key is `AIza` plus 35 characters, so a value of any other
   shape is a different kind of credential entirely (an OAuth token, a service
   account, a Vertex key) and no amount of looking at Google will explain it.
   Four characters of a 39-character secret is not a secret, and this row is
   behind the office's own door in any case. */
const KEY_LEN = 39, KEY_HEAD = "AIza";
function keyShape() {
  const k = apiKey();
  if (!k) return null;
  return { len: k.length, head: k.slice(0, 4),
           looksRight: k.length === KEY_LEN && k.slice(0, 4) === KEY_HEAD };
}
...
async function ask(opts) {
  ...
  if (!res.ok) {
    ...
    /* THE FAULT IS REPORTED WHERE IT BELONGS. A rejected key arriving as "the
       ...
       above said the key was working (§117). */
...
module.exports = { ask, configured, model, KEY_NAME, corpusText, instruction, SCHEMA,
                   looksLikeBadKey, HANDOFF_LINE, keyShape, KEY_LEN, KEY_HEAD };
