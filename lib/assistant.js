...
/* Whether this provider accepts thinkingConfig — null until asked, then
   remembered for the process (§133). Never persisted: a fresh process asks
...
/* RETIRED FOR NEW USERS, NOT FOR OLD ONES (§131). Google's own 404, verbatim
...
/* 20s, AND THE FUNCTION AROUND IT MUST OUTLIVE IT (§132). This was 12s while
...
/* WHAT KEY IS THIS, WITHOUT SAYING WHAT KEY THIS IS (§126). "The key is
   ...
   TWO SHAPES ARE GOOGLE'S (§131.2, correcting §126 the day it met a real key):
...
async function ask(opts) {
  ...
  /* NO THINKING FOR A LOOKUP (§133). The model reasons before it answers by
     ...
     is §132's timeout fault coming back however big the budget. Answering
  ...
  const cfg = {
    ...
    temperature: 0,
    /* THINKING COUNTS (§132): reasoning is billed against maxOutputTokens, so
  ...
  if (!res.ok) {
    ...
    /* THE KNOB, NOT THE QUESTION (§133). A 400 that names thinking is the
...
module.exports = { ask, configured, model, KEY_NAME, corpusText, instruction, SCHEMA,
                   ...
                   /* test hook only: the cap's memory is per process, and a test
                      that cannot reset it can only measure the first case (§133). */
