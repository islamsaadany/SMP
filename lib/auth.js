...
/* ── IDENTITY LIVES IN THE PLATFORM SCHEMA (spec 042) ─────────────────
...
/* ── SIGNED IN, AND NOBODY ON THIS CLIENT'S REGISTER (§313.32) ────────
...
/* The signed-in person — a session whose account or seat no longer exists
...
async function getSession(client, req, clientKey) {
  ...
  /* ── SIGNED IN, AND NOT FOR THIS CLIENT (spec 042) ───────────────
  ...
  /* ── THE SEAT IS NOT THE QUESTION FOR AN OFFICE ACCOUNT (§313.20) ──
  ...
  /* ── AND WHEN THAT KEY IS NOBODY, THE ADDRESS IS ASKED (§313.32) ──────
     ...
     build nothing ever creates that row (§313.30) — so the key named nobody
  ...
  /* `role` is the SEAT role only — super / gceo / cceo. Owner, custodian and
     function head are read from what points at the person (§33), on both
     sides through lib/rules.js, and every save is authorised against the
     stored graph in lib/authorize.js (§42). */
...
module.exports = { hashPassword, verifyPassword, passwordPolicy,
                   seatIn, destroySessionsFor, notThisClient,
