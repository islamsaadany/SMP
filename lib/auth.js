...
/* ── SIGNED IN, AND NOBODY ON THIS CLIENT'S REGISTER (§288.32) ────────
...
async function getSession(client, req, clientKey) {
  ...
  /* ── THE SEAT IS NOT THE QUESTION FOR AN OFFICE ACCOUNT (§288.20) ──
  ...
  /* ── AND WHEN THAT KEY IS NOBODY, THE ADDRESS IS ASKED (§288.32) ──────
     ...
     build nothing ever creates that row (§288.30) — so the key named nobody
