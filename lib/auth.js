...
async function seatIn(client, clientKey, email) {
  const r = await client.query(
    "SELECT person_key, is_super FROM " + T_ACCOUNT_CLIENTS + " WHERE email = $1 AND client_key = $2",
...
async function getSession(client, req, clientKey) {
  ...
  const a = await client.query(
    "SELECT s.email, a.name, a.kind, a.role AS office_role, a.must_change " +
  ...
  if (!clientKey) {
    ...
    return { key: null, email: acct.email, name: acct.name, kind: acct.kind,
             officeRole: acct.office_role, mustChange: !!acct.must_change };
  }
  ...
  if (acct.kind !== "client") {
    ...
    const world = {
      mine: (await client.query(
        "SELECT client_key, person_key, is_super FROM " + T_ACCOUNT_CLIENTS + " WHERE email = $1",
        [acct.email])).rows,
    ...
    const account = { email: acct.email, role: acct.office_role, kind: acct.kind, status: "active" };
  ...
  return { key: p.person_key, name: p.name, role: p.role, unit: p.unit_key,
           ...
           email: p.email, kind: p.kind, isSuper: !!p.is_super };
}
