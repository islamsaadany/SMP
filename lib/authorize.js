...
function collect(stored, incoming, w) {
  ...
  /* ── AND ONE OF THE FIVE IS NOT SPOKEN FOR THE WHOLE SUBJECT (§287) ───
  ...
  /* ── AND TAKING A DRAFT'S LOCK OFF IS NOT SPEAKING FOR THE SUBJECT
     ...
     owner roles (§147.7, §287) are affected by a park and hold no Reopen, so
...
/* ── WHAT A "DONE" MARK CAN BE ABOUT (§287) ────────────────────────────
...
function authorize(stored, incoming, person) {
  ...
  changes.forEach(function (ch) {
    ...
    switch (ch.kind) {
      ...
      /* ── TAKING A DRAFT'S LOCK OFF (§287.6) ───────────────────────────
