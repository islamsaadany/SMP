...
/* §253: which slides a review does not show. Its own kind rather than a
...
function collectUnit(key, su, iu, add, w) {
  ...
  /* §253, ahead of the settings compare and deliberately not part of it:
...
/* §253: everything FN_KNOWN speaks for, plus the field classified on its own.
...
function collectFunction(key, sf, iff, add, w) {
  ...
  /* §253. Classified HERE and not inside collectUnit(), because `asUnit()`
...
function authorize(stored, incoming, person) {
  ...
  changes.forEach(function (ch) {
    ...
    switch (ch.kind) {
      ...
      /* ── WHICH SLIDES A REVIEW SHOWS (§253) ─────────────────────────
