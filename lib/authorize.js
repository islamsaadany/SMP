...
function collect(stored, incoming) {
  const out = [];
  ...
  const gExtra = GROUP_OWN.concat(["capabilities", "branding"]);
  ...
  unitKeys.forEach(function (k) {
    collectUnit(k, (stored.units || {})[k], (incoming.units || {})[k], add);
  });
...
function addFigures(unitKey, storedUnit, rows, what, add) {
  const mine = [], byOwner = {};
  ...
  rows.forEach(function (r) {
    ...
    if (stored && stored.src && stored.src.by) {
      (byOwner[stored.src.by] = byOwner[stored.src.by] || []).push(
        Object.assign({}, r, { src: stored.src }));
    } else {
      mine.push(r);
    }
  });
  ...
  Object.keys(byOwner).forEach(function (who) {
    add("sourceReporting", unitKey, what + " sourced from " +
        (byOwner[who][0].src.team || who), byOwner[who]);
  });
}

function collectUnit(key, su, iu, add) {
  ...
  splitRows(su.keyObjectives, iu.keyObjectives, FIGURE,
    function (rows) { addFigures(key, su, rows, "key objective figures", add); },
  ...
  splitRows(su.keyObjectives, iu.keyObjectives, SRC,
    function (rows) { add("setup", key, "who reports a key objective's figure", rows); },
    function () {});
  ...
  if (!same(idsOf(su.items), idsOf(iu.items))) {
    ...
    Object.keys(sm).forEach(function (id) {
      ...
      splitRows(a.measures, b.measures, FIGURE,
        function (rows) { addFigures(key, su, rows, "reported figures", add); },
        function () {});
      ...
      splitRows(a.measures, b.measures, SRC,
        function (rows) { add("setup", key, "who reports a figure", rows); },
        function () {});
...
function authorize(stored, incoming, person) {
  ...
  const changes = collect(stored, incoming);
  const refusals = [];
  ...
  changes.forEach(function (ch) {
    ...
    switch (ch.kind) {
      ...
      /* §16.7. The figure belongs to the team that is master of the number,
         and to the person named in it — in ANY unit, without owning any of
         them. The unit's own people cannot move it: that is the whole point,
         because a figure entered by the person measured against it is not a
         source of anything. */
      case "sourceReporting": {
        ...
        if (smo) return;
        const notMine = (ch.rows || []).filter(function (r) {
          return !r.src || r.src.by !== person.key;
        });
        if (notMine.length) {
          const src = (ch.rows || [])[0] && (ch.rows || [])[0].src;
          no("That figure is reported by " + ((src && src.team) || "another team") +
             " — " + t + " does not enter it.");
        }
        return;
      }
