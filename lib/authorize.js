...
const FN_KNOWN = FN_SETUP.concat(UNIT_FOUNDATION,
  ["key", "items", "keyObjectives", "swot", "perf", "exec"]);
...
function asUnit(f, ukey) {
  return { ukey: ukey, items: f.items || [], keyObjectives: f.keyObjectives || [],
...
function authorize(stored, incoming, person) {
  ...
  changes.forEach(function (ch) {
    ...
    switch (ch.kind) {
      ...
      /* ── THE STRATEGY TAB IS THE OFFICE'S (§93) ──────────────────
         A plan ARRIVES by upload and is corrected by the SMO alone (§22,
         §31); §93 read that argument on to the rest of the tab, because the
      ...
      /* THE FUNCTION'S HALF OF THE SAME TAB (§93, §53.5). A capability's
