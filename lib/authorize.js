...
/* `pend` is §132's pending-fill marks — known here so a mark the gap pass
...
/* ── FILL THE GAPS: THE PASS THAT RUNS BEFORE THE DIFF (§132, spec 021) ──
...
function collectUnit(key, su, iu, add, w) {
  ...
  /* §132: extract and classify the pending-fill transitions first, applied
...
function asUnit(f, ukey) {
  ...
  /* §132: the pending-fill marks ride through, or a function's aspiration
...
function collectCapabilities(sList, iList, add) {
  ...
  Object.keys(sm).forEach(function (id) {
    ...
    /* §132: the gap pass, on a clone, exactly as collectUnit runs it — a
