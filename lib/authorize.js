...
function collectUnit(key, su, iu, add) {
  ...
  splitRows(su.keyObjectives, iu.keyObjectives, REPORT.unitKO,
    function (rows) { add("unitReporting", key, "key objective figures", rows); },
  ...
  if (!same(idsOf(su.items), idsOf(iu.items))) {
    ...
    Object.keys(sm).forEach(function (id) {
      ...
      splitRows(a.measures, b.measures, REPORT.measure,
