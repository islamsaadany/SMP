...
/* ── THE TENANT THAT WAS ALREADY HERE (§147.21) ──────────────────────
...
/* ── An explicit reset, once (Islam, 2026-09-01) ─────────────────────
   ...
   `must_change` IS FALSE (§147.25, Islam: "keep my password ... and 1234
...
async function ensureOfficeRow(client, seat, account, madeHere) {
  ...
  if (found.rowCount) {
    /* ── A ROW THAT WAS ALREADY THERE IS ADOPTED, NOT REWRITTEN (§147.29) ──
  ...
  /* ── AND ON A CLIENT THAT ALREADY HAS A REGISTER, NOTHING IS CREATED
        (§147.30, Islam: "make the access from the platform match the raya
     ...
     account is (§147.29), several accounts may say the same row, and somebody
  ...
  /* WHOSE REGISTER IS IT (§147.31). On a client the platform BUILT, its team
