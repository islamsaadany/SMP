/* THE PLATFORM'S OWN I/O — the registry, the office's accounts, and the one
   ...
   3 · A CLIENT'S CONNECTION WEARS THAT CLIENT'S BADGE (§303.35). Rules 1 and
...
async function pointAt(client, schema) {
  /* THE ONE SESSION-LEVEL STATEMENT IN THE PRODUCT, AND IT IS ARGUED FOR
     (§303.34). A transaction pooler does not keep this between statements, so
  ...
  await client.query("SET search_path TO " + ident(schema)); /* session-state-ok: §303.34 — direct connection, see getPool() */
}
    ...
    if (!SAID_NO_CREATEROLE) {
      ...
      try {
        console.warn("[db] the database key cannot create roles, so client requests are served " +
          ...
          "database, only by the platform (§303.35). Give the key CREATEROLE.");
...
async function wearBadge(client, schema) {
  ...
  if (b.badged) {
    /* THE SECOND SESSION-LEVEL STATEMENT IN THE PRODUCT, argued for by the
       same paragraph as the first (§303.34): a direct connection is one
       backend for the life of the checkout, and takeBadgeOff() resets it. */
    await client.query("SET ROLE " + b.role); /* session-state-ok: §303.35 — direct connection, reset at release, see getPool() */
  }
  return b;
}
...
async function connectFor(pg, slug) {
  ...
  try {
    ...
    /* THE SCHEMA IS BROUGHT UP TO DATE BEFORE THE BADGE GOES ON (§303.35):
...
/* ── A CONNECTION FOR THE DOOR (spec 030, §303.13) ─────────────────
...
/* ── A WAY IN (§303.14, and §43.1's own rule one level out) ──────────
...
/* WHO AN OFFICE ACCOUNT IS INSIDE A CLIENT — minted once from the address,
   ...
   again the moment an admin opens a client nobody has been put on (§303.20),
...
/* ── THE TENANT THAT WAS ALREADY HERE (§303.21) ──────────────────────
...
/* ── An explicit reset, once (Islam, 2026-09-01) ─────────────────────
   ...
   `must_change` IS FALSE (§303.25, Islam: "keep my password ... and 1234
...
async function createClientSchema(pg, schemaName, orgName) {
  ...
  return withOwner(pg, name, async function (c) {
    ...
    /* The badge is made with the room (§303.35), so a client created from the
...
async function ensureOfficeRow(client, seat, account, madeHere) {
  ...
  if (found.rowCount) {
    /* ── A ROW THAT WAS ALREADY THERE IS ADOPTED, NOT REWRITTEN (§303.29) ──
  ...
  /* ── AND ON A CLIENT THAT ALREADY HAS A REGISTER, NOTHING IS CREATED
        (§303.30, Islam: "make the access from the platform match the raya
     ...
     account is (§303.29), several accounts may say the same row, and somebody
  ...
  /* WHOSE REGISTER IS IT (§303.31). On a client the platform BUILT, its team
