/* THE PLATFORM'S OWN I/O — the registry, the office's accounts, and the one
   ...
   3 · A CLIENT'S CONNECTION WEARS THAT CLIENT'S BADGE (§313.35). Rules 1 and
       2 are OUR code being right. This one is the database refusing when it
       is not: every connection serving a client request does `SET ROLE` to a
       role that may see that client's schema and nothing else, so a query
       that names another client's room — by mistake, by a stale path, by an
       injection that got past ident() — is refused by Postgres, not by us.
       The owner's key is still the only key, and it still creates the rooms;
       what changes is that it never SERVES a request wearing its own rights. */
...
async function pointAt(client, schema) {
  /* THE ONE SESSION-LEVEL STATEMENT IN THE PRODUCT, AND IT IS ARGUED FOR
     (§313.34). A transaction pooler does not keep this between statements, so
     `getPool()` asks for the DIRECT connection first — where a checked-out
     client is one backend for the life of the checkout, which is exactly what
     the reset in `withSchema()` has always assumed. The cost of that (a real
     connection per checkout) and the bigger fix it defers (every client request
     in one transaction with SET LOCAL) are both written down there. */
  await client.query("SET search_path TO " + ident(schema)); /* session-state-ok: §313.34 — direct connection, see getPool() */
}

/* ── THE BADGE (§313.35) ─────────────────────────────────────────────
   One database role per client schema, `smp_<schema>`, that can neither log
   in nor create anything: it may use ITS schema's tables and sequences, and
   the handful of shared platform tables the door needs (below), and nothing
   else. The owner is made a member of it, so a connection checked out as the
   owner can `SET ROLE` into it — and while it wears the badge, Postgres judges
   every statement as the badge. `RESET ROLE` takes it off at release, beside
   the `RESET search_path` that was already there.

   WHY THIS SHAPE AND NOT A KEY PER CLIENT: a key per client is one more secret
   in Vercel per client, one more open connection per client, and "add a
   client" stops being a press on the Clients page. The badge is made by the
   platform itself the moment a room is made, needs no password because it
   can only be worn from inside by somebody already holding the owner's key,
   and costs one statement per request. What it does NOT do is protect against
   the owner's key leaking — neither would keys-per-client held in the same
   place — and the shared platform tables the door reads are reachable from
   every badge, because sign-in lives there (stated in PLATFORM_FOR_CLIENTS).

   CREATED LAZILY AND REMEMBERED PER PROCESS, exactly as ensureReady is: the
   first client request of a process makes sure the badge exists and holds the
   grants, under a transaction-scoped lock (§289) so two cold starts cannot
   both CREATE ROLE. Re-granting on ALL TABLES every process start is what
   covers a table a migration added since; ALTER DEFAULT PRIVILEGES covers one
   added later in this process's life.

   IF THE HOST WILL NOT LET THE OWNER MAKE A ROLE (no CREATEROLE), the request
   is served as the owner — exactly as every request was served before this
   section — and the process says so ONCE in the runtime log. A boundary that
   takes every client down to prove a point is the wrong boundary (§61); one
   that quietly degrades is not much better, which is why it is not quiet. */
function badgeRoleFor(schema) { return ident("smp_" + ident(schema)); }
/* The owner's name is the host's to choose (Neon's is `neondb_owner`, a
   local one may be anything), so it is quoted rather than validated. */
function quoted(name) { return '"' + String(name).replace(/"/g, '""') + '"'; }

/* What a CLIENT request may touch in the platform schema, and how. Sign-in,
   the session, a person's account and the password states are all shared
   rows read through a client's connection (api/auth.js, lib/auth.js), so the
   badge holds these and only these. Anything a client path touches in
   `platform` that is not on this list fails with 42501 — loudly, on purpose. */
const PLATFORM_FOR_CLIENTS = {
  clients:         "SELECT",
  platform_access: "SELECT",
  accounts:        "SELECT, INSERT, UPDATE",
  account_clients: "SELECT, INSERT, UPDATE, DELETE",
  sessions:        "SELECT, INSERT, DELETE",
  login_attempts:  "SELECT, INSERT, DELETE"
};

const BADGE_LOCK = 420044;             /* beside 420042 (state-io) and 420043 */
const BADGED = new Map();              /* schema → promise of { badged, role } */
let SAID_NO_CREATEROLE = false;
function forgetBadged(schema) { if (schema) BADGED.delete(schema); else BADGED.clear(); }

function ensureBadge(client, schema) {
  const key = ident(schema);
  const held = BADGED.get(key);
  if (held) return held;
  const p = ensureBadgeOnce(client, key).catch(function (e) {
    BADGED.delete(key);               /* a failed grant must not be remembered */
    throw e;
  });
  BADGED.set(key, p);
  return p;
...
async function ensureBadgeOnce(client, schema) {
  const role = badgeRoleFor(schema);
  /* The owner's own name, asked of the connection rather than read from the
     URL: the GRANT below names it, and `current_user` is exact. */
  const who = (await client.query("SELECT current_user AS u, current_setting('server_version_num')::int AS v")).rows[0];
  const can = (await client.query(
    "SELECT rolcreaterole OR rolsuper AS ok FROM pg_roles WHERE rolname = current_user")).rows[0];
  if (!can || !can.ok) {
    if (!SAID_NO_CREATEROLE) {
      SAID_NO_CREATEROLE = true;
      try {
        console.warn("[db] the database key cannot create roles, so client requests are served " +
          "without a badge — one client's connection is not fenced from another by the " +
          "database, only by the platform (§313.35). Give the key CREATEROLE.");
      } catch (e) { /* not worth failing a request for */ }
    }
    return { badged: false, role: null };
  }
  await client.query("BEGIN");
  try {
    await client.query("SELECT pg_advisory_xact_lock($1, hashtext($2))", [BADGE_LOCK, schema]);
    const there = await client.query("SELECT 1 FROM pg_roles WHERE rolname = $1", [role]);
    if (!there.rowCount) await client.query("CREATE ROLE " + role + " NOLOGIN NOINHERIT");
    /* Membership WITH SET is what lets the owner wear it. Postgres 16 stopped
       granting SET to a role's creator by default (createrole_self_grant), so
       it is said outright there; the option does not exist before 16. */
    await client.query("GRANT " + role + " TO " + quoted(who.u) + (who.v >= 160000 ? " WITH SET TRUE" : ""));
    await client.query("GRANT USAGE ON SCHEMA " + schema + " TO " + role);
    await client.query("GRANT ALL ON ALL TABLES IN SCHEMA " + schema + " TO " + role);
    await client.query("GRANT ALL ON ALL SEQUENCES IN SCHEMA " + schema + " TO " + role);
    await client.query("ALTER DEFAULT PRIVILEGES IN SCHEMA " + schema + " GRANT ALL ON TABLES TO " + role);
    await client.query("ALTER DEFAULT PRIVILEGES IN SCHEMA " + schema + " GRANT ALL ON SEQUENCES TO " + role);
    await client.query("GRANT USAGE ON SCHEMA " + PLATFORM_SCHEMA + " TO " + role);
    for (const t of Object.keys(PLATFORM_FOR_CLIENTS)) {
      await client.query("GRANT " + PLATFORM_FOR_CLIENTS[t] + " ON " + PLATFORM_SCHEMA + "." + t + " TO " + role);
    }
    await client.query("COMMIT");
  } catch (e) {
    try { await client.query("ROLLBACK"); } catch (e2) { /* going away anyway */ }
    throw e;
  }
  return { badged: true, role: role };
}

/* Put the badge on: made if missing, then worn. Returns what was worn so a
   caller can say so (connectFor marks the connection). */
async function wearBadge(client, schema) {
  const b = await ensureBadge(client, schema);
  if (b.badged) {
    /* THE SECOND SESSION-LEVEL STATEMENT IN THE PRODUCT, argued for by the
       same paragraph as the first (§313.34): a direct connection is one
       backend for the life of the checkout, and takeBadgeOff() resets it. */
    await client.query("SET ROLE " + b.role); /* session-state-ok: §313.35 — direct connection, reset at release, see getPool() */
  }
  return b;
}

async function takeBadgeOff(client) {
  /* RESET even when the body threw, and the ROLE before the PATH: a released
     connection still wearing a client's badge would serve the next request
     as that client — refused where it is somebody else's, which is at least
     loud — and one still pointed at its schema would serve it silently. */
  try { await client.query("RESET ROLE"); } catch (e) { /* the connection is going away anyway */ }
  try { await client.query("RESET search_path"); } catch (e) { /* the connection is going away anyway */ }
}

/* THE ONE DOOR TO A CLIENT'S SCHEMA, WEARING ITS BADGE. Everything that
   touches a client's tables on the platform's own behalf goes through here,
   so there is exactly one place that can forget to reset. The platform schema
   has no badge (it is the owner's own), and creating or migrating a schema is
   owner's work — those two callers use withOwner(). */
async function withSchema(pg, schema, fn) {
  const client = await getPool(pg).connect();
  try {
    await pointAt(client, schema);
    if (schema !== PLATFORM_SCHEMA) await wearBadge(client, schema);
    return await fn(client);
  } finally {
    await takeBadgeOff(client);
    client.release();
  }
}

/* THE OWNER'S DOOR: the same connection with no badge on, for the two things
   a badge cannot do — create a schema's tables and migrate them. Every other
   caller wants withSchema(). */
async function withOwner(pg, schema, fn) {
  const client = await getPool(pg).connect();
  try {
    await pointAt(client, schema);
    return await fn(client);
...
async function connectFor(pg, slug) {
  ...
  try {
    ...
    /* THE SCHEMA IS BROUGHT UP TO DATE BEFORE THE BADGE GOES ON (§313.35):
       ensureReady creates tables and runs migrations, which the badge may not
       do, so it runs here as the owner — memoised per process, so on a warm
       instance it is one Map lookup. The endpoints still call it themselves
       and hit the same memo; only the order changed. */
    await io.ensureReady(client, row.schema_name);
    const b = await wearBadge(client, row.schema_name);
    client._smpBadged = !!b.badged;
  } catch (e) {
    await takeBadgeOff(client);
    client.release();
    throw e;
...
/* ── A CONNECTION FOR THE DOOR (spec 042, §313.13) ─────────────────
...
async function ensurePlatformReadyOnce(client) {
  ...
  /* ── ONE TRANSACTION, AND THE LOCK IS TRANSACTION-SCOPED (§289, from main,
     one schema out) ──────────────────────────────────────────────────
     Production talks to Neon through PgBouncer in transaction mode, where a
     statement sent OUTSIDE a transaction may run on any backend — so a session
     advisory lock is taken on a backend the next statement never sees, and the
     unlock in the `finally` is sent to whichever backend answers next. It
     guards nothing and it leaks. `scripts/test-session-state.js` arrived from
     main with exactly that rule made executable, and it went red on this file.

     The whole bootstrap is one transaction: the lock is held for it and
     released by the COMMIT, and a failure anywhere leaves the platform schema
     as it was rather than half-built. */
  await client.query("BEGIN");
  try {
    await client.query("SELECT pg_advisory_xact_lock($1)", [PLATFORM_LOCK]);
    /* SET LOCAL, so the path lasts exactly as long as the transaction and
       cannot be handed to the next request on a pooled backend. Callers point
       the connection themselves either side of this (withSchema before it,
       connectPlatform after it), so nothing outside depends on it. */
    await client.query("SET LOCAL search_path TO " + PLATFORM_SCHEMA);
    ...
    try { await client.query("ROLLBACK"); } catch (e2) { /* the connection is going away anyway */ }
    throw e;
  }
}

/* ── A WAY IN (§313.14, and §43.1's own rule one level out) ──────────
...
/* WHO AN OFFICE ACCOUNT IS INSIDE A CLIENT — minted once from the address,
   ...
   again the moment an admin opens a client nobody has been put on (§313.20),
...
/* ── THE TENANT THAT WAS ALREADY HERE (§313.21) ──────────────────────
...
/* ── An explicit reset, once (Islam, 2026-09-01) ─────────────────────
   ...
   `must_change` IS FALSE (§313.25, Islam: "keep my password ... and 1234
...
async function createClientSchema(pg, schemaName, orgName) {
  ...
  return withOwner(pg, name, async function (c) {
    /* seed:false — the client's own name, and NOTHING else (spec §7.2). */
    const r = await io.ensureReady(c, name, { seed: false, orgName: orgName || "" });
    /* The badge is made with the room (§313.35), so a client created from the
       Clients page is fenced from its first request — not from its first
       cold start on some later process. */
    await ensureBadge(c, name);
    return r;
  });
}

/* ── The office's row inside a client (spec 042 §6) ───────────────
...
async function ensureOfficeRow(client, seat, account, madeHere) {
  ...
  if (found.rowCount) {
    /* ── A ROW THAT WAS ALREADY THERE IS ADOPTED, NOT REWRITTEN (§313.29) ──
  ...
  /* ── AND ON A CLIENT THAT ALREADY HAS A REGISTER, NOTHING IS CREATED
        (§313.30, Islam: "make the access from the platform match the raya
     ...
     account is (§313.29), several accounts may say the same row, and somebody
  ...
  /* WHOSE REGISTER IS IT (§313.31). On a client the platform BUILT, its team
...
module.exports = {
  ...
  getPool, withSchema, withOwner, withPlatform, pointAt,
  badgeRoleFor, ensureBadge, forgetBadged, PLATFORM_FOR_CLIENTS,
  ...
  adoptExistingTenant, ORIGINAL_SCHEMA,
  officePersonKey,
  ensureOfficeRow,
