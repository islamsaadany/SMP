...
(function (root, factory) {
  ...
  /* ── RETIRING A CONSULTANT, AND DELETING ONE (§338) ──────────────────
     Islam: *"allow me to delete a consultants from a user so I can remove
     noran"* — two accounts one letter apart, and the list was ADD-ONLY: it
     names, renames, re-addresses, hands out passwords and sets the admin
     flag, and had no way to take anybody off it. §61's trap with half of it
     already built, because `saveConsultant` has accepted a `status` since it
     was written and no control ever sent one.

     RETIRE IS THE DEFAULT AND DELETE IS THE EXCEPTION, which is §35 and §62
     one level up: a consultant who has left Forefront should stop being able
     to sign in, and the record of which clients they worked on is worth
     keeping. Deleting is for the row that should never have existed.

     DELETING NEEDS THEM RETIRED FIRST — §323's rule for a client, and for
     the same reason: it is the guard rather than a second confirmation, so
     nobody is deleted while still able to sign in, and the two presses are
     separated by a state somebody had to choose.

     THE TARGET IS WHAT DECIDES, NEVER THE ACTOR ALONE (§89's own shape for
     issuing a password): never yourself — a platform you have just deleted
     your own way into is not one anybody can run — and never an admin, whose
     flag comes off on this same page first, deliberately, so the two acts
     stay apart.

     AND WHAT IT CANNOT SAY IS SAID BY THE SERVER: whether they still hold a
     seat on a client is a question about rows, so it is asked there and the
     refusal NAMES the clients (§62, §16.7). A rule that guessed at it here
     would be a second answer to a question the database owns. */
  function mayRetireConsultant(world, account, target) {
    if (!mayManageConsultants(world, account)) return false;
    if (!target || target.email === account.email) return false;
    return !target.is_admin;
  }
  /* Destruction is the platform admin's outright (§89), which NARROWS the
     line above rather than copying it: retiring somebody can be handed to
     whoever manages consultants, and removing them from the record cannot. */
  function mayDeleteConsultant(world, account, target) {
    if (!isAdmin(account)) return false;
    if (!target || target.email === account.email) return false;
    if (target.is_admin) return false;
    return target.status === "retired";
  }
  ...
  return {
    ...
    mayRetireConsultant: mayRetireConsultant, mayDeleteConsultant: mayDeleteConsultant,
