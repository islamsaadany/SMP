...
var GAP_OPTIONAL = { tactic: ["collaborators"],
  ...
  var CHAT_DEFAULTS = Object.freeze({
    ...
    /* ── ONE CHASE PER CONVERSATION, NOT ONE PER MESSAGE (§261) ────
       Islam: *"when someone send to me when I don't reply it send an email
       for each message — it needs to compile some messages rather than an
       email for each message."*

       THIS IS THE COMPILING, AND IT IS NOT A DIGEST. There is no scheduler
       on Vercel (§97.5 settled that and it still holds), so the only moment
       anything can be decided is the moment a message arrives — nothing can
       hold five messages back and send one summary at the end of the hour.
       What CAN be decided at that moment is whether an email about this
       conversation has already gone out and been left unanswered, and that
       is the whole rule: the first message chases, the rest are silent, and
       what does go carries the WHOLE waiting spell rather than one line.

       THE NUMBER IS THE SECOND CHASE, NOT THE FIRST. A conversation that is
       still waiting this many minutes after the last email may chase again —
       so a message written the next morning is not swallowed by an email
       sent yesterday, which is the one thing a plain "once and never again"
       gets wrong (option A, put to Islam with that cost and refused). */
    quiet: 60
  });
  ...
  /* THE FLOOR IS FIVE MINUTES AND THE REASON IS THE FEATURE: below that the
     quiet period is shorter than the time it takes somebody to write a second
     message, so the flood this exists to stop comes straight back. A day is
     the ceiling, because past that the chase has stopped being a chase. */
  var CHAT_QUIET_MIN = 5, CHAT_QUIET_MAX = 1440;
  ...
  function chatCfg(raw) {
    ...
    return {
      ...
      /* THE SAME READER AS `away`, FOR THE SAME REASON (§169, §104.10):
         `Number(null)` and `Number("")` are both 0 and both finite, so a
         clamp alone would answer FIVE MINUTES for every tenant that has
         never set this — a quiet period so short that the flood it exists
         to stop is barely narrowed. Absence is tested for before the value
         is read as a number. */
      quiet:     (function (v) {
                   if (v === null || v === undefined || v === "") return CHAT_DEFAULTS.quiet;
                   var n = Math.round(Number(v));
                   if (!isFinite(n)) return CHAT_DEFAULTS.quiet;
                   return Math.max(CHAT_QUIET_MIN, Math.min(CHAT_QUIET_MAX, n));
                 })(c && c.quiet),
  ...
  /* ── MAY THIS CONVERSATION CHASE AGAIN? (§261) ─────────────────────
     ONE DEFINITION, THREE ASKERS: the office's chase, the away chase to a
     person, and the line that tells the office what pressing Send will do
     before they press it (§97.5 — a screen that predicts and a server that
     decides separately is the drift this module exists to prevent).

     `last` is when an email about THIS conversation last went out — cleared
     the moment the other side answers, so a fresh spell always chases and
     the quiet period only ever silences a repeat.

     IT FAILS TOWARDS SENDING. Nothing here, a value that will not read as a
     time, a clock that disagrees — all answer YES, because the two failures
     are not equal: one email too many is a nuisance, and a question nobody
     is told about is the whole feature not working (§35's shape — absence
     must never be reported as an answer). */
  function chatChaseDue(last, now, quietMinutes) {
    if (last === null || last === undefined || last === "") return true;
    var t = (last instanceof Date) ? last.getTime()
          : (typeof last === "number") ? last
          : Date.parse(String(last));
    if (!isFinite(t)) return true;
    var n = (now instanceof Date) ? now.getTime()
          : (typeof now === "number" && isFinite(now)) ? now : Date.now();
    var q = chatCfg({ quiet: quietMinutes }).quiet;
    return (n - t) >= q * 60000;
  }
  ...
  return {
    ...
    CHAT_QUIET_MIN: CHAT_QUIET_MIN, CHAT_QUIET_MAX: CHAT_QUIET_MAX,
    chatCfg: chatCfg, chatBeat: chatBeat, chatChaseDue: chatChaseDue,
