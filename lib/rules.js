...
(function (root, factory) {
  ...
  /* `fill` sits between view and edit (§132, spec 021): everywhere the
  ...
  var GAP_FIELDS = {
    ...
    /* `quarters` is VIRTUAL and covers q1–q4 as one mark: a tactic set to
       ...
       `collaborators` joined at Islam's direction (§132.10, reversing
  ...
  /* The fields on this row still holding NOTHING — blank and unmarked. A
     ...
     (§132.12: one definition, or the counts disagree with the fields). */
  ...
  /* u_anal is a strategy page with NO fillable field (the SWOT is rows,
     and rows are never fill mode's — §132.2), so the fill grant must not
     draw its pen: a pen that opens nothing is §61's trap wearing §132's
  ...
  function namedOn(row, person) {
    ...
    /* §132.10: A PENDING NAME CONFERS NOTHING. Being named here is what
