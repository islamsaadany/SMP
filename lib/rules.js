...
var GAP_OPTIONAL = { tactic: ["collaborators"],
  ...
  /* ── AN ACTION HALF WAY THROUGH ITS WINDOW (§296) ──────────────────
     Islam, of a yes/no action running Q2 and Q3 and reported after Q2:
     *"if we report mid way it's either done or not and that is not fair"*.

     REPRODUCED BEFORE ANYTHING WAS PROPOSED, on the running platform: at the
     end of June the reporter's three choices were BLANK (which the row counts
     as unanswered, so Submit is refused), NO (which scores 0 — and the red
     then demands a note explaining a failure that has not happened) and YES
     (untrue). The only answer that could honestly be given was the one that
     marks the unit down for work that is on track.

     A THIRD ANSWER, AND ITS PER-CENT IS OWED. `In progress` on its own is the
     row saying it has started and not saying how far, which is NOT an answer:
     it scores null, so the row stays unanswered, Submit stays shut and the
     reporting bar names it — exactly what a project milestone's In progress
     with no per-cent already does (§104.10), rather than a second rule.

     THE MARKER IS STORED, NEVER INFERRED FROM THE NUMBER. Reading "any number
     here means in progress" was the smaller shape and it silently
     reinterprets what tenants already hold: §257.2 KEEPS a figure when a row
     is switched to Y/N, so a measure reported `28%` and later made yes/no
     still stores "28%" — not scored today, and scored 56 by a rule that took
     a bare number as the answer. A value nobody re-entered must not change
     what it scores (§96.2), so the word has to be present for it to count.

     NO NEW FIELD, AND THAT IS WHY THE SERVER NEEDS NOTHING. The answer stays
     in the one field it has always been in (`actual`, `outActual`), which
     `lib/authorize.js` already classifies as reporting — so there is nothing
     to migrate and no rule to widen (asserted rather than assumed, §172). */
  var YN_WIP = "In progress";
  function ynIsWip(v) {
    return String(v == null ? "" : v).trim().toLowerCase()
             .indexOf(YN_WIP.toLowerCase()) === 0;
  }
  function ynPct(v) {
    if (!ynIsWip(v)) return null;
    var m = String(v).trim().slice(YN_WIP.length).match(/-?[\d.]+/);
    if (!m) return null;
    var n = parseFloat(m[0]);
    return isNaN(n) ? null : Math.max(0, Math.min(100, n));
  }
  /* Said it has started and did not say how far. The row is OUTSTANDING, not
     forgiven: the same shape `statusPending()` answers for a milestone. */
  function ynPending(v) {
    return ynIsWip(v) && ynPct(v) == null;
  }
  function ynScore(v, share) {
    var a = ynAnswer(v);
    if (a != null) return a ? 100 : 0;
    var p = ynPct(v);
    if (p == null) return null;
    /* MEASURED AGAINST THE PART OF ITS OWN WINDOW THAT HAS PASSED — Islam's
       own call, put to him with the arithmetic first: *"it should read 120 as
       60% would lower the progress value while it's not due in Q2"*. So 60%
       done at the half-way point reads 120, ahead of plan, and not 60.

       THE SHARE IS SUPPLIED, NEVER RE-DERIVED (§250): a tactic's outcome
       passes the share of its own quarters, a key measure or an objective —
       which has no window of its own — passes the year's. One arithmetic told
       which period it is measuring; a second copy is how the score and the
       benchmark printed beside it start disagreeing (§53.5).

       ABSENT MEANS THE WHOLE PERIOD, so a plan whose dates cannot be read
       reads the figure as it stands rather than refusing to score it at all
       (`tacticShare`'s own rule, kept). A share of NOUGHT is not due yet, and
       not due yet is not nought (§35, §276): the row leaves every average. */
    var due = share == null ? 100 : share * 100;
    if (due <= 0) return null;
    return Math.max(0, Math.min(150, Math.round(p / due * 100)));
  }
  /* What a part-way answer is read against, as a whole per cent — the same
     share `ynScore` divides by, so the column and the score cannot disagree
     about one row. Null where there is nothing to be due yet. */
  function ynDue(share) {
    if (share == null) return null;
    var d = Math.round(share * 100);
    return d > 0 ? d : null;
  }
  /* WHAT TO STORE. The answer and its per-cent are two controls writing ONE
     field, so picking `In progress` when a number has already been given must
     not throw that number away — a control that destroys what somebody typed,
     with no undo, is worse than the state it was fixing (§96.2). */
  function ynCommit(stored, picked) {
    if (picked !== YN_WIP) return picked;
    var p = ynPct(stored);
    return p == null ? YN_WIP : YN_WIP + " " + p;
  }
  function ynCommitPct(pct) {
    var s = String(pct == null ? "" : pct).replace(/[^0-9.]/g, "");
    if (s === "") return YN_WIP;           /* cleared: still in progress, owed */
    var n = parseFloat(s);
    return isNaN(n) ? YN_WIP : YN_WIP + " " + Math.max(0, Math.min(100, n));
  }
  /* How a stored answer READS, wherever one is printed — one reader, or the
     page and the projector spell the same answer differently (§53.5). */
  function ynShown(v) {
    if (!ynIsWip(v)) return String(v == null ? "" : v);
    var p = ynPct(v);
    return p == null ? YN_WIP : YN_WIP + " — " + p + "%";
  }
  ...
  return {
    ...
    YN_WIP: YN_WIP, ynIsWip: ynIsWip, ynPct: ynPct, ynPending: ynPending,
    ynDue: ynDue, ynCommit: ynCommit, ynCommitPct: ynCommitPct, ynShown: ynShown,
