...
var GAP_OPTIONAL = { tactic: ["collaborators"],
  ...
  var CHAT_DEFAULTS = Object.freeze({
    ...
    /* ── HOW LONG BEFORE AN UNREAD REPLY IS CHASED (§283) ─────────
       MINUTES, and a DIFFERENT question from `away` above it. `away` asks
       "is this person at their desk right now"; this asks "have they come
       back and read it". Islam's number, chosen from a range with the
       reasoning stated: short enough to reach them the same morning, long
       enough that somebody who steps out for coffee and reads it on their
       return is not emailed about a message they have already answered. */
    chase: 30,
  ...
  /* The chase's own limits, named once beside the value they bound so the
     box, the clamp and the sentence under it cannot disagree (§169). */
  var CHAT_CHASE_MIN = 5, CHAT_CHASE_MAX = 1440;
  ...
  function chatCfg(raw) {
    ...
    return {
      ...
      chase:     (function (v) {
                   if (v === null || v === undefined || v === "") return CHAT_DEFAULTS.chase;
                   var n = Number(v);
                   if (!isFinite(n)) return CHAT_DEFAULTS.chase;
                   return Math.max(CHAT_CHASE_MIN, Math.min(CHAT_CHASE_MAX, Math.round(n)));
                 })(c && c.chase),
      away:      (function (v) {
  ...
  return {
    ...
    CHAT_CHASE_MIN: CHAT_CHASE_MIN, CHAT_CHASE_MAX: CHAT_CHASE_MAX,
