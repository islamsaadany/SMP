...
(function (root, factory) {
  ...
  var PAGES = [
    ...
    /* Who is master of which figure — Setup, so the SMO alone (§16.7). */
    { key:"c_source", area:"a_setup", scope:"setup", label:"Source of figures", note:"Which numbers another team enters, and who enters them" },
  ...
  /* ── 5 · Source of a figure (§16.7) ───────────────────────────────
     Many measures are not the business unit's number at all. Revenue, margin
     and most financial figures already exist in Finance before a unit is asked
     for them, and asking the unit to type them achieves two bad things at
     once: the same figure is entered ten times, and it can be entered WRONG
     ten times, because the person typing is not the person who knows it.

     So a key objective or a measure may carry a SOURCE — a team, and the
     person in it who is master of that number:

         row.src = { team: "<function key>", by: "<person key>" }

     The team is stored rather than read off the person, because the person may
     sit with the SMO rather than in the function whose number it is — Islam's
     "Finance SMO custodian", who is not the supporting function's own
     custodian. The attribution has to name the number's home, not the
     reporter's desk.

     THE FIGURE MOVES, THE NOTE DOES NOT. The source enters actual and
     progress; the UNIT always writes the note. The number is the source's,
     the performance is the unit's, and the explanation belongs to whoever owns
     the performance (§16.7, settled).

     THIS IS A RULE, NOT A MATRIX CELL. A source's reach is defined entirely by
     what it is named on — it crosses units without owning them, and it reaches
     nothing it is not named on. A column whose every cell holds the same
     answer is a question with no second answer (§37). */
  function isSourced(row) { return !!(row && row.src && row.src.by); }
  function sourcedBy(row, person) {
    return isSourced(row) && person && row.src.by === person.key;
  }

  /* Every sourced figure in the tenant, flattened. Small by construction —
     a tenant has tens of measures, not thousands — and the callers all want
     the same shape: which unit, which row, whose. */
  ...
  function sourcesFor(w, person) {
    if (!person) return [];
    return sourceRows(w).filter(function (r) { return r.src.by === person.key; });
  }
  function ownsSources(w, person) { return sourcesFor(w, person).length > 0; }

  /* Every figure custodian in the tenant, grouped. There will be several —
     Finance is the first, not the only one — so the SMO needs one place that
     says who owes what rather than a page that answers for one team at a time.
     Keyed on team AND person, because two people in the same team may each be
     master of different numbers. */
  function sourceParties(w) {
    const by = {}, out = [];
    sourceRows(w).forEach(function (r) {
      const k = r.src.team + "|" + r.src.by;
      if (!by[k]) { by[k] = { team: r.src.team, by: r.src.by, rows: [] }; out.push(by[k]); }
      by[k].rows.push(r);
    });
    return out;
  }
  ...
  return {
    ...
    isSourced: isSourced, sourcedBy: sourcedBy, sourceRows: sourceRows,
    sourcesFor: sourcesFor, ownsSources: ownsSources, sourceParties: sourceParties,
    rowById: rowById
  };
});
