...
(function (root, factory) {
  ...
  var GAP_FIELDS = {
    ...
    /* `quarters` is VIRTUAL and covers q1–q4 as one mark: a tactic set to
       ...
       `collaborators` joined at Islam's direction (§145.10, reversing
       this section's own first exclusion): an EMPTY list is a gap, an
       existing list is never the filler's — and because being named is
       what lets a Contributor report the line (§50.2), a pending name
       CONFERS NOTHING until the office confirms: namedOn() below skips
       any field still wearing the mark. */
    tactic:  ["owner", "collaborators", "quarters"],
    capko:   ["dir", "target", "compile", "weight"],
    project: ["owner", "start", "end"]
  };
  /* The fields a SCORE reads. A row with one of these pending is not
     scored — the comparison is not ready, so it reads null and leaves the
     average the way an unmeasured outcome already does (§104.10) — and
     Submit waits on them, because submitting says "performance can be
     read" (Islam, 2026-08-27: report and draft freely, never submit past
     a pending target). A pending owner, date or 3-year target gates
     nothing: none of them changes what this cycle's figure means. */
  var GAP_SCORE_FIELDS = ["dir", "target", "compile", "weight", "quarters"];
  ...
  function gapMissing(kind, row) {
    return (GAP_FIELDS[kind] || []).filter(function (f) {
      if (pendOf(row)[f]) return false;
      if (f === "quarters") return quartersBlank(row);
      return gapBlank(row[f]);
    });
  }
  function pendingScore(row) {
    var p = pendOf(row);
    for (var i = 0; i < GAP_SCORE_FIELDS.length; i++)
      if (p[GAP_SCORE_FIELDS[i]]) return true;
    return false;
  }
  ...
  function namedOn(row, person) {
    ...
    /* §145.10: A PENDING NAME CONFERS NOTHING. Being named here is what
       lets a Contributor report the line, so a name the custodian filled
       but the office has not confirmed is visible on the page and silent
       in this rule — the same shape as the score waiting on the target.
       Field by field, because an owner confirmed long ago must not lose
       their line over a collaborator still pending beside them. */
    var pd = pendOf(row);
    var hits = (pd.owner ? [] : [row.owner]).concat(
      !pd.collaborators && Array.isArray(row.collaborators) ? row.collaborators : []);
  ...
  return {
    ...
    GAP_FIELDS: GAP_FIELDS, GAP_SCORE_FIELDS: GAP_SCORE_FIELDS,
    gapBlank: gapBlank, quartersBlank: quartersBlank,
    pendOf: pendOf, pendingScore: pendingScore, mayFillPage: mayFillPage,
