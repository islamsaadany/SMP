...
/* ── ONE TRANSACTION, AND THE LOCK THAT LIVES INSIDE IT (§289) ─────────
   ...
   AND THE LOCK IS STILL PER SCHEMA (spec 030). One lock for the whole
...
/* ── AND THE DIRECT CONNECTION COMES FIRST (§303.34) ──────────────────
   THE ORDER OF THIS LIST IS A DECISION NOW, not a spelling.

   Every client's tables live in a schema of their own (§303), and a request is
  ...
  if (!pick || !pick.direct) {
    ...
    try {
      console.warn("[db] no direct (unpooled) connection string is set" +
        ...
        "Set DATABASE_URL_UNPOOLED in the project's settings (§303.34).");
