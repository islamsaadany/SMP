...
async function writeState(client, state, opts) {
  ...
  try {
    /* ── A SAVE CLEARS WITHOUT SHUTTING EVERYBODY ELSE OUT (§288) ─────
       This was `TRUNCATE ... CASCADE`, and TRUNCATE takes an ACCESS
       EXCLUSIVE lock on every table it names for the WHOLE of §240's
       transaction — so while any save is running, anywhere in the tenant,
       every reader of the graph is not slow but FROZEN. Measured on a real
       Postgres with a save held open:

                                        TRUNCATE      DELETE
         auth.getSession()               blocked        2ms
         the chat's settings (org)        blocked        1ms
         the register                     blocked        1ms
         a unit's plan                    blocked        1ms
         the clear itself                  44ms        116ms

       §282 FIXED ONE READER AND THIS IS WHY THAT WAS NOT ENOUGH. That
       section took the register's name out of the chat's queue, which was
       true and useful and left three doors standing in front of it — the
       schema check, the settings read, and `getSession`, which joins
       `people` and therefore sits in front of EVERY authenticated request
       in the product. Patching readers one at a time was chasing a fault
       with more heads than could be counted; this is the one change that
       answers all of them, because DELETE takes ROW EXCLUSIVE, which does
       not conflict with a reader at all.

       WHAT TRUNCATE WAS DOING THAT THIS MUST STILL DO — checked against
       the schema this repo builds rather than assumed:
         · all 14 foreign keys into these tables are ON DELETE CASCADE, so
           nothing is left behind whatever order the list is in;
         · no table outside the list references one inside it, so the chat
           and the message record stay outside the clear exactly as before
           (§97, §146) — the only FKs into them are their own siblings';
         · there are no user triggers, so DELETE's row triggers and
           TRUNCATE's statement trigger are both nothing;
         · the statement carried no RESTART IDENTITY and there are no
           sequences here — every id is a text key — so nothing is reset
           by one and not the other.

       THE COST IS STATED AND MEASURED, NOT HOPED FOR. About 72ms added to
       a clear that a save already surrounds with hundreds; and dead rows,
       which is the one that needed watching, because a graph rewritten on
       every save is a great deal of churn. It is BOUNDED — 160 full saves
       against the worked example, vacuumed between rounds:

         before                    5.5 MB
         after  40 full saves     15.4 MB
         after  80 full saves     15.5 MB
         after 120 full saves     15.5 MB
         after 160 full saves     15.5 MB

       It steps up once and then flattens, because the space a DELETE frees
       is reused rather than lost. VACUUM does not hand it back to the disk
       and does not need to.

       AND THIS IS THE FALLBACK PATH, NOT EVERY SAVE — worth stating exactly,
       or a later reader measures a plain edit, finds it never comes here, and
       doubts the whole account. §241's incremental writer has been LIVE on
       production since 2026-09-03, and its per-subject deletes take ROW
       EXCLUSIVE, so they never blocked a reader in the first place. What
       still falls back to this clear is every settings, register, reorder and
       add/remove change, and every whole-graph post from a tab on an older
       build.

       WHICH FITS THE REPORT RATHER THAN WEAKENING IT: a new build reloads
       every browser at once, every one of them posts the whole graph, and
       Neon has usually gone to sleep — so the fallback path is taken by the
       whole tenant within a minute of a deploy, which is exactly when Islam
       saw the chat die. */
    for (const t of ALL_TABLES) await client.query("DELETE FROM " + t);
    ...
    /* ── A DELETED PERSON'S DOOR GOES WITH THEM (§69) ───────────────
       `people` is cleared and rewritten on every save (§288 made that a
       DELETE where it was a TRUNCATE; the rest of this paragraph is
       unchanged by that and is why). The four tables that make up an
       identity are NOT cleared: credentials, sessions, bu_declarations and
       login_attempts live outside the state graph deliberately (§19, §56)
       and carry NO foreign key to people — a FK would take the whole table
       with it on the clear, and a column on `people` would be erased by it.
    ...
    for (const t of ["credentials", "sessions", "bu_declarations"]) {
      ...
      /* ASKED WITHOUT A SCHEMA ON PURPOSE (spec 042). This read
...
/* ── WRITE ONLY THE SUBJECTS THAT CHANGED (2026-09-01) ────────────────────
   LIVE ON PRODUCTION since 2026-09-03 (SMP_INCREMENTAL_WRITE=1, switched on
   in another session) — this comment said "dark / off by default" until
   2026-09-04, which is §104.8's fault: a comment describing an intention the
   deployment had already moved past. So MOST SAVES NOW TAKE THIS PATH and
   never reach writeState's clear at all; what still falls back to it is named
   under `planSubjects` below.

   writeState rewrites all 31 tables. This is the incremental alternative: from the change list the client already sends (§210/§215), work
...
/* ── ONCE PER PROCESS, NOT ONCE PER REQUEST (§98.1) ──────────────────────
   ...
   true yet. */
/* ── ONE PROMISE PER SCHEMA, NOT ONE PER PROCESS (spec 042) ──────────
...
/* `opts.seed === false` is how a NEW CLIENT is created (spec 042 §7.2): apply
...
/* ── ONE TRANSACTION, AND THE LOCK THAT LIVES INSIDE IT (§289) ─────────
   This took `pg_advisory_lock` — a SESSION lock — and then ran the schema,
   both migration phases and the bootstrap credential as separate statements.
   Production is Neon behind PgBouncer in transaction pooling, where every
   statement outside a transaction may run on a DIFFERENT backend: the lock
   was taken on one backend, the migrations ran on another, and the unlock
   landed wherever it landed. So the lock protected nothing exactly when it
   was needed — a deploy carrying a new migration file, which reloads every
   open tab through the version banner and starts several instances cold in
   the same second. Two of them both ran the migration; the second's
   `INSERT INTO _sql_migrations` hit the primary key (23505) and the sign-in
   page said "Something went wrong". Islam, twice in one day, once per
   deploy. §240 learned this for the SAVE and this function never caught up.

   THE WHOLE BOOTSTRAP IS ONE TRANSACTION NOW, under the transaction-scoped
   lock, which pins one backend for its life (§240's own argument). A second
   cold start waits at the lock, and when the first COMMITs it reads the
   registry and finds the migration already recorded. It also makes a
   migration atomic: a half-applied one rolls back instead of leaving the
   tenant between two shapes. Every statement here is transactional
   (checked: no CONCURRENTLY, no VACUUM, no CREATE DATABASE anywhere in
   db/), and the seed is written with { inTransaction: true } so it does not
   open a second transaction inside this one.

   AND THE LOCK IS STILL PER SCHEMA (spec 042). One lock for the whole
   database would make two clients' cold starts queue behind each other for
   no reason, and on a serverless platform that is two requests waiting on
   work neither needs. `pg_advisory_xact_lock` takes the same two-argument
   form as the session lock it replaces, so the key is unchanged — what
   changes is that it now dies with the transaction rather than with a
   session PgBouncer may hand to somebody else.

   THE HEAL STAYS OUTSIDE, after the COMMIT, for the reason it already had:
   it may never shut the door, and inside a transaction one failed statement
   would abort the bootstrap with it. It is caught and logged as before. */
async function ensureReadyOnce(client, schema, opts) {
  await client.query("BEGIN");
  let seeded = false;
  try {
    await client.query("SELECT pg_advisory_xact_lock($1, hashtext($2))", [LOCK_KEY, schema]);
    const dir = dbDir();
    ...
    if (!r.rowCount && opts && opts.seed === false) {
      /* A CLIENT WITH A NAME AND NOTHING ELSE. readState answers null without
         ...
         — which is exactly what "real but empty" means (spec 042 §3). */
      await client.query(
      ...
      await writeState(client, seed, { inTransaction: true });
      seeded = true;
    }
    ...
    /* NO BOOTSTRAP SEAT IN A CLIENT CREATED FROM THE OUTER PLATFORM (spec 042
  ...
  /* §260, after the seed and both migration phases: it reads the graph the
     ...
     and NOT recorded, so the next boot tries again. Outside the transaction
     above on purpose (§289): it opens its own, and a failure inside the
     bootstrap's would take the bootstrap down with it. */
  /* NAMED, BECAUSE THE MESSAGE HAD STOPPED BEING TRUE (§324.4, §104.8). It
     read *"one-line titles did not run"* whichever of these threw, and there
     are three of them now — so a failure in the newest one sent whoever read
     the log to the oldest. Each is named as it runs; the control flow is
     unchanged, and they still stop at the first failure, which is what leaves
     the rest unrecorded and tried again on the next boot. */
  let healing = "one-line titles";
  try {
    await healOneLineTitles(client);
    healing = "the review month";
    await healReviewMonth(client);
    healing = "a function's own projects";
    await healOwnProjects(client);
  } catch (e) {
    console.error("[heal] " + healing + " did not run:", e && e.message);
  }
...
async function resetTheSMOPassword(client, auth) {
  ...
  if (done.rowCount) return;
  /* GUARDED ON THE TENANT IT WAS WRITTEN FOR (spec 042), for the same reason
...
function cleanOneLine(state) {
  ...
  /* A PROJECT IS THE SAME SHAPE WHEREVER IT IS HELD (§329, §53.5). This walk
     was written inside the capabilities loop, and §322 moved every tenant's
     projects onto the FUNCTION — so from that version until this one the heal
     reached none of them, silently: a pasted title keeps its blank lines, the
     pen draws a 643px box for one sentence (§260) and nothing says why. One
     walk, asked of both holders, or the two drift again the first time either
     gains a field. */
  const projects = function (ps) {
  ...
  (state.functionKeys || []).forEach(function (k) {
    const f = (state.functions || {})[k];
    planned(f);
    /* A function that plans in PROJECTS holds them directly (§322). */
...
/* ── ONE MONTH, ONE MEANING (§307) ────────────────────────────────────
   ...
   Two fields answered one question. `review.to` decided which rows were ASKED
   for; `review.extra.asOfMonth` decided what they were MEASURED against — and
   they could disagree, which on his own tenant they did: a cycle covering to
   Jun 2027 reporting as of Aug 2026. The second field is gone and its meaning
   is in `to`.

   THE DIRECTION IS THE SAFETY ARGUMENT, and it is why the heal runs this way
   round rather than the other. Every score on the platform is computed from
   the review point TODAY, so carrying that month into `to` moves NO SCORE.
   Healing the other way — leaving `to` alone and calling it the review point —
   would have re-measured every figure in the tenant against a different month.
   What does change is which work is asked for: anything dated between the old
   `to` and the new one moves in or out of this cycle's reporting, which is
   stated on the page rather than discovered.

   IT WRITES FOUR DIGITS, through the same `monthLabel` the browser draws with
   (§42): `cycleYear()` scrapes a four-digit year out of `to`, and a two-digit
   one there would take `to` out of that scrape — §239.3's fault landing on the
   field this change makes load-bearing.

   A MONTH THAT CANNOT BE READ IS LEFT ALONE. The key is still removed (nothing
   reads it any more, so keeping it would be a second copy of a fact with no
   reader), and `to` keeps whatever it holds — never overwritten with something
   the platform could not parse in the first place (§96.2).

   ARCHIVES AND CLOSED CYCLES ARE NOT TOUCHED: `history` keeps a score, never a
   review point, and `plan_archives` is the record of what was uploaded. */
const MERGE_REVIEW_MONTH = "043-the-cycle-end-is-the-review-point.js";

function mergeReviewMonth(state) {
  const R = require("./rules.js");
  const rv = state.review;
  if (!rv || !Object.prototype.hasOwnProperty.call(rv, "asOfMonth")) return 0;
  const t = R.whenMonths(rv.asOfMonth, false, null);
  const label = R.monthLabel(t);
  let n = 0;
  if (label && label !== rv.to) { rv.to = label; n = 1; }
  delete rv.asOfMonth;
  return n || 1;   /* the key leaving is itself a change worth writing */
}

async function healReviewMonth(client) {
  const done = await client.query("SELECT 1 FROM _sql_migrations WHERE name = $1", [MERGE_REVIEW_MONTH]);
  if (done.rowCount) return 0;
  const state = await readState(client);
  const n = mergeReviewMonth(state);
  if (n) await writeState(client, state);
  await client.query("INSERT INTO _sql_migrations (name) VALUES ($1)", [MERGE_REVIEW_MONTH]);
  return n;
}

/* ── A FUNCTION'S PROJECTS ARE ITS OWN (§322, spec 046 stage 1) ───────────
   Islam, of what his own tenant holds: *"they caapbilities in raya trade are
   not capabilities they are just projecst under functions."*

   So every box a tenant is holding is dissolved into the function that
   carries it: the projects move, the definition and the key objectives move
   only where the function has none of its own, and the box goes. The rule is
   `SMPRules.dissolvePlan` and is the SAME one the Remove dialog presses
   (§42) — two answers to what this act means is how the control and the
   one-off drift, and the drift would be silent because both would run.

   IT ARCHIVES EACH BOX FIRST (§49.2, §232), for the one thing genuinely lost:
   the grouping and its NAME. Nothing else goes — every project keeps its id,
   its rows and every figure reported against it — so a tenant can still read
   what its containers were called.

   NO SCHEMA CHANGE. `functions` maps six columns and files every other key
   into `extra`, so `projects` on a function travels the road `items`,
   `keyObjectives`, `swot` and `def` already travel (§118, §213). Proved
   against a real Postgres by scripts/test-functional-projects.js rather than
   claimed here (§172 — that claim has been wrong in this file before).

   A FUNCTION THAT PLANS IN PILLARS IS UNTOUCHED, and so is a capability whose
   function is missing or retired-to-nothing: it is left standing rather than
   dissolved into a function that cannot draw it (§96.2). */
const OWN_PROJECTS = "045-a-functions-projects-are-its-own.js";

const ARCHIVE_MONTHS = ["Jan","Feb","Mar","Apr","May","Jun",
                        "Jul","Aug","Sep","Oct","Nov","Dec"];
function archiveToday() {
  const d = new Date();
  return d.getDate() + " " + ARCHIVE_MONTHS[d.getMonth()] + " " + d.getFullYear();
}
function ownProjects(state) {
  const R = require("./rules.js");
  const g = state.group || {};
  ...
  caps.forEach(function (c) {
    const f = c && c.fn ? fns[c.fn] : null;
    if (!f || String(f.format) === "pillars") { keep.push(c); return; }
    if (!Array.isArray(f.projects)) f.projects = [];
    if (!Array.isArray(f.keyObjectives)) f.keyObjectives = [];
    const mv = R.dissolvePlan(c, f);
    /* AN ARCHIVE OF NOTHING IS NOT A RECORD, IT IS CLUTTER — and that is not a
       judgement call, it is what `archiveCapPlan` already does on the other
       side: it refuses an empty plan outright (`planIsEmpty`). Without this a
       FRESH deployment writes one per box, because migration 004's clean slate
       has just emptied every one of them, so a brand-new client would open on
       eight archives of containers it never had (§53.5: follow what the
       neighbour does rather than deciding it again). */
    const holds = (c.projects || []).length || (c.keyObjectives || []).length;
    if (holds) (state.archives = state.archives || []).unshift({
      id: "arc-" + OWN_PROJECTS + "-" + c.id,
      kind: "cap", key: c.id, name: c.name,
      /* The platform's own date shape, never an ISO string: this row sits in
         the same list as every archive a person made, and one entry spelled
         differently reads as a different kind of thing (§53.5). */
      at: archiveToday(), by: "the platform",
      why: "before \u201c" + (c.name || "a capability") + "\u201d became " +
           (f.name || "the function") + "\u2019s own projects",
      counts: { projects: (c.projects || []).length,
                objectives: (c.keyObjectives || []).length },
      plan: { def: c.def, keyObjectives: c.keyObjectives || [], projects: c.projects || [] }
    });
    if (mv.def != null) f.def = mv.def;
    if (mv.keyObjectives) f.keyObjectives = mv.keyObjectives;
    /* PUSHED, never assigned: another capability of the same function may
       already have moved its projects here, and a fresh array would throw the
       first lot away. */
...
async function healOwnProjects(client) {
  const done = await client.query("SELECT 1 FROM _sql_migrations WHERE name = $1", [OWN_PROJECTS]);
  if (done.rowCount) return 0;
  const state = await readState(client);
  const n = ownProjects(state);
  if (n) await writeState(client, state);
  await client.query("INSERT INTO _sql_migrations (name) VALUES ($1)", [OWN_PROJECTS]);
  return n;
}
...
/* ── AND THE DIRECT CONNECTION COMES FIRST (§313.34) ──────────────────
   THE ORDER OF THIS LIST IS A DECISION NOW, not a spelling.

   Every client's tables live in a schema of their own (§313), and a request is
   pointed at one with `SET search_path`. Behind Neon's POOLED endpoint that is
   PgBouncer in transaction mode, where a statement sent outside a transaction
   may run on any backend — so the SET can land on one backend and the query
   that depends on it on another, which at best is "relation does not exist"
   and at worst is a request reading against whatever schema the previous
   request left on that backend. §289 is the same hazard with a lock instead of
   a setting; `scripts/test-session-state.js` is the rule made executable, and
   it is red on `pointAt()` for exactly this reason.

   ON A DIRECT CONNECTION THE HAZARD DOES NOT EXIST — a checked-out client is
   one backend for the life of the checkout, which is what `withSchema()`'s
   set-and-reset has always assumed. So the unpooled names are asked for FIRST.

   THE COST IS STATED: a direct connection is a real connection, and a database
   holds a bounded number of them. With a handful of consultants and one
   client's people that is nowhere near the limit; a great many people saving in
   the same seconds is where it would bite, and the answer THEN is to make every
   client request one transaction with `SET LOCAL` — correct at any scale, and a
   change to every endpoint, which is not a thing to do in the same week as a
   live migration (rule 1b).

   THE POOLED NAMES STAY AS THE FALLBACK, deliberately: a deployment that has
   only those must still start. It says so once, because a deployment silently
   running on the risky endpoint is the one thing worse than the risk. */
const DIRECT_URL_NAMES = ["DATABASE_URL_UNPOOLED", "POSTGRES_URL_NON_POOLING"];
const POOLED_URL_NAMES = ["DATABASE_URL", "POSTGRES_URL", "POSTGRES_PRISMA_URL",
                          "NEON_DATABASE_URL"];

function pickDbUrl() {
  for (const n of DIRECT_URL_NAMES) if (process.env[n]) return { url: process.env[n], name: n, direct: true };
  for (const n of POOLED_URL_NAMES) if (process.env[n]) return { url: process.env[n], name: n, direct: false };
  return null;
}
...
function getPool(pg) {
  ...
  tuneTypes(pg);
  const pick = pickDbUrl();
  const url = pick && pick.url;
  if (!pick || !pick.direct) {
    /* Said once per process, and never with the address in it. */
    try {
      console.warn("[db] no direct (unpooled) connection string is set" +
        (pick ? " — using " + pick.name : "") + ". A client's schema is selected with " +
        "SET search_path, which a transaction pooler does not keep between statements. " +
        "Set DATABASE_URL_UNPOOLED in the project's settings (§313.34).");
    } catch (e) { /* a log that will not write is not worth failing a request for */ }
  }
  if (!url) {
...
module.exports = { writeState, writeStateIncremental, planSubjects,
                   readState, ensureReady, forgetReady, tuneTypes, getPool, pickDbUrl,
                   cleanOneLine, mergeReviewMonth, ownProjects, healOwnProjects };
