...
async function writeState(client, state, opts) {
  ...
  try {
    ...
    for (const t of ["credentials", "sessions", "bu_declarations"]) {
      ...
      /* ASKED WITHOUT A SCHEMA ON PURPOSE (spec 024). This read
...
/* ── ONCE PER PROCESS, NOT ONCE PER REQUEST (§98.1) ──────────────────────
   ...
   true yet. */
/* ── ONE PROMISE PER SCHEMA, NOT ONE PER PROCESS (spec 024) ──────────
...
/* `opts.seed === false` is how a NEW CLIENT is created (spec 024 §7.2): apply
  ...
  try {
    ...
    if (!r.rowCount && opts && opts.seed === false) {
      /* A CLIENT WITH A NAME AND NOTHING ELSE. readState answers null without
         ...
         — which is exactly what "real but empty" means (spec 024 §3). */
      await client.query(
    ...
    /* NO BOOTSTRAP SEAT IN A CLIENT CREATED FROM THE OUTER PLATFORM (spec 024
...
async function resetTheSMOPassword(client, auth) {
  ...
  if (done.rowCount) return;
  /* GUARDED ON THE TENANT IT WAS WRITTEN FOR (spec 024), for the same reason
