...
const E = {
  ...
  /* No due and no owner — a deliverable never had either to say (§53.4). And
     no `actual`: it is "yes"/"no" on a binary deliverable and a NUMBER on a
     percentage one, so it rides in `extra` (jsonb, which keeps 77 a 77) rather
     than in the text column beside it. See the note in db/schema.sql. */
  deliverables:  { table: "deliverables",
    cols: { id:"id", name:"name", kind:"kind", note:"note" } },
...
/* ── ONCE PER PROCESS, NOT ONCE PER REQUEST (§96.1) ──────────────────────
   ...
   four seconds (§95). Measured before the guard: 14 round trips per poll, of
   which 10 were this.
