...
async function ensureReadyOnce(client, schema, opts) {
  ...
  /* NAMED, BECAUSE THE MESSAGE HAD STOPPED BEING TRUE (§328.4, §104.8). It
...
function cleanOneLine(state) {
  ...
  /* A PROJECT IS THE SAME SHAPE WHEREVER IT IS HELD (§333, §53.5). This walk
     was written inside the capabilities loop, and §326 moved every tenant's
  ...
  (state.functionKeys || []).forEach(function (k) {
    ...
    planned(f);
    /* A function that plans in PROJECTS holds them directly (§326). */
...
/* ── A FUNCTION'S PROJECTS ARE ITS OWN (§326, spec 048 stage 1) ───────────
