...
async function writeState(client, state, opts) {
  ...
  try {
    /* ── A SAVE CLEARS WITHOUT SHUTTING EVERYBODY ELSE OUT (§288) ─────
       ...
       and does not need to. §241's incremental writer keeps most saves off
       this path entirely; what still comes through it is every settings,
       register, reorder and add/remove change, and every whole-graph post
       from a tab on an older build — which is exactly the moment Islam was
       reporting, since a new build reloads every browser at once. */
