#!/usr/bin/env node
/* WHAT THE DESTINATION ROW OFFERS, AND WHY (§383).
 *
...
const supplied = {
  ...
  /* §382's My reporting tab, ANSWERED HERE RATHER THAN RUN, and saying which
     ...
     reported §383's own fault on a build that does not have it: 10 units and
     ...
     assertion below pass while proving nothing about this tab (§113.8):
       ...
       always a tab that ignores its gate, which MUST move the row or this
              harness cannot see §383's fault through a `when` at all.
...
/* A CHECK THAT DIES REPORTS NOTHING (§215). Measured against a build from
   before §383 every later section would throw on the first missing gate and
...
if (GATES.some((n) => typeof box[n] !== "function")) {
  console.log("\nnav-row: " + ok + " ok, " + bad.length + " failed" +
    "\n  this build predates \u00a7383 \u2014 the gates it measures are not in it, " +
...
console.log("\n§5  the My reporting tab does not make a destination reachable either (§382)");
LIBRARY_ON = true;
...
check("...while a tab that ignored its gate WOULD move it, so this can still " +
  "see §383's fault through a `when` (§113.8)", ALWAYS !== OFF);
