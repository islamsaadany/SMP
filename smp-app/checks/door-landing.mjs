...
await section("9 · Insights' Setup: Access writes a grant the door reads (spec 056 §4.4)", async () => {
  ...
  try {
    ...
    /* AND EVERY OTHER ROW THIS PERSON HOLDS \u2014 ASKED, NEVER TYPED
       ...
       differ. \u00a7379 made a tactic's owner a role, so the Mobile head derives
    ...
    check(heldRows.length >= 2 && heldRows.includes("owner"),
          "\u2026and the Mobile head holds more than the one row, so shutting one is not shutting them (\u00a733, \u00a7379)", JSON.stringify(heldRows));
