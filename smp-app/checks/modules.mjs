...
console.log("\n4c · who may open a module (§359.5)");
{
  ...
  /* SHUT EVERY ROW THIS PERSON HOLDS, ASKED RATHER THAN TYPED (§218,
     ...
     and §382 made a tactic's owner a role, so this unit's head derives
  ...
  check("the unit head holds more than the one row, so shutting one is not shutting them (§33, §382)",
...
/* THE SHELL'S OWN BODY AND STYLESHEET, served over HTTP — `route.js` returns
   ...
   platform. It IS mounted from a paint since §381, so the stub carries a
...
/* AND THE STUB HAS TO CARRY A `paint`, BECAUSE THE SWITCHER IS A PAINT-TIME
   CONTROL NOW (§381). It was an IIFE built at load, and the comment above
   still said route.js "builds it whether or not the app has hydrated" — true
   until §381 moved it, because the list has to ask whether the tab row
