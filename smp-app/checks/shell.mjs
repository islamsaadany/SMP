...
await section("3b \u00b7 the client's own settings wear the client's bar (\u00a7362, spec 058)", async () => {
  ...
  /* AND A THIRD, BECAUSE THE SECOND IS ON THE TAB ROW (\u00a7380). REWRITTEN,
     NEVER LOOSENED (\u00a7218, \u00a7214.3): with exactly `strategy` and
     `insights` the switcher below is correctly NOT drawn \u2014 \u00a7380 drops a
     ...
     decision \u00a7380 made is guarded rather than merely worked around. */
  ...
  /* AND THE OTHER END OF \u00a7380'S OWN RULE, MADE (\u00a794.2, \u00a7255). Take the
     ...
     row already reaches goes red here, which is the fault \u00a7380 removed
  ...
  check(two.switcher === false && two.row === true,
        "\u2026and with the reports as the only other module it is NOT drawn \u2014 the tab row already reaches them (\u00a7380, \u00a732)", JSON.stringify(two));
  await owner.query("update tenants set modules = $1 where id = $2", [JSON.stringify(["strategy", "insights", "tracker"]), tenantId]);
  await open("/raya-trade/strategy/setup/cycle");
...
await section("3d · the reports tab is stamped with what this client HAS (§382)", async () => {
  /* THE SEAM, AND IT WAS MEASURED BY NOBODY (§316.7's own finding, one
     feature over). §382 has two halves and each is asserted in its own
