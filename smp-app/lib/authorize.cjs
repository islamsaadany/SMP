...
/* §266: which subjects a master presentation runs through, and in what order.
   Its own kind rather than a group setting, for HIDE_SLIDES' reason above:
   both land on the office, so this changes no permission — it changes the
...
function collect(stored, incoming, w) {
  ...
  TOP_SETUP.forEach(function (k) {
    ...
    if (k === "people") {
      ...
      /* ── THE OFFICE'S OWN ROWS ARE SET OUTSIDE THIS CLIENT (spec 042) ──
         A Forefront person's row is written from the client's configuration on
         the outer platform, and it carries `extra.forefront`. Editing it from
         inside the client would give one fact two masters, and the next visit
         would overwrite whichever was typed here — a change that appears to
         work and silently un-happens is worse than one that is refused.

         REFUSED WHOLESALE rather than classified: `officeRow` is its own kind,
         so the refusal says where the change belongs (§16.7) instead of
      ...
      if (officeTouched.length)
        add("officeRow", null, "a Forefront row (" + officeTouched.join(", ") +
            ") — set on that client's configuration, not here");
  ...
  /* ── AND ONE OF THE FIVE IS NOT SPOKEN FOR THE WHOLE SUBJECT (§301) ───
  ...
  const DONE = "done";
  /* ── AND TAKING A DRAFT'S LOCK OFF IS NOT SPEAKING FOR THE SUBJECT
     (§301.6) ────────────────────────────────────────────────────────────
     Islam, of a project owner frozen by somebody else's Save draft: *"project
     owner can reopen eventually he can only report or fill missing for his
     own project."*

     §220 made a saved draft CLOSE the whole report, at his own instruction,
     and at the time everybody who could park also held Reopen. The bounded
     owner roles (§147.7, §301) are affected by a park and hold no Reopen, so
     one press froze every one of them until the custodian came back.

     UNPARKING IS ITS OWN KIND, and only unparking: parking, submitting, the
     note and the slides all still speak for the whole subject and stay
     `reportState`. Nothing was sent, so nothing is retracted — the lock comes
     off and the custodian may park it again.

     RETRACTING A SUBMISSION IS DELIBERATELY NOT HERE, and the reason is
     stated rather than assumed: `submitted` leaving still classifies as
     `reportState`, so one project's owner cannot pull back a report the
     office has already received on behalf of every other project in it.
     His sentence rests on the owner being limited to their own project,
     which is true of reporting and filling and is NOT true of a submission. */
  const UNPARK = function (a, b, t) {
    return !!a[t] && !Object.prototype.hasOwnProperty.call(b, t);
  };
  perTarget.forEach(function (field) {
    ...
    Object.keys(a).concat(Object.keys(b)).forEach(function (t) {
      ...
      /* §380: an owner locking their OWN lines in one subject. Never
  ...
  /* §380: whether reporting follows the Owner column. SETUP, and the same
  ...
  /* ── THE PLANNING PERIOD (§308) ─────────────────────────────────
     ...
     halves of one pen is how a screen comes to offer what the save refuses
     (§42). NAMED, so the refusal says which control rather than "the group's
     planFrom" (§16.7) — and named as ONE change however many of the two
...
/* §380: A TACTIC WHOSE OWNER ENTERS IT. `addFigures` above is the same split
   ...
   §385: AND A NAME THAT REACHES NOBODY IS NOT AN OWNER. `lineOwned` asks
...
function gapPassUnit(target, su, iu, add) {
  ...
  Object.keys(sm).forEach(function (id) {
    ...
    /* §382 wrapped pctx here to carry the tactic's own Owner for its role;
       §385 removed the role, so a tactic is filled through the pillar's ctx
...
function collectCapabilities(sList, iList, add) {
  ...
  Object.keys(sm).forEach(function (id) {
    ...
    /* §278.3: AND A REORDER IS A REORDER HERE TOO. This walk had no third
       callback, so moving a capability's objectives fell through to `capPlan`
       — the office's — while the identical act on a UNIT's objectives has
       classified as `arrange` since §101. One question, two answers, and the
       screen now draws a handle on both: a custodian dragging a row on a
       supporting function's Overview would have watched it move and the save
       come back refused, which is §94.3's exact fault. */
...
/* ── WHAT A "DONE" MARK CAN BE ABOUT (§301) ────────────────────────────
...
function authorize(stored, incoming, person) {
  ...
  changes.forEach(function (ch) {
    ...
    switch (ch.kind) {
      ...
      /* ── SET SOMEWHERE ELSE, SO REFUSED EVERYWHERE HERE (spec 042) ──
         Nobody inside a client edits a Forefront person's row: it is written
         from that client's configuration on the outer platform, and the next
         visit would overwrite anything typed here. Refused for EVERY role,
         the office's own included — this is not a permission, it is a fact
         about where the row comes from, and the message says where to go. */
      case "officeRow":
        no("That row is set on this client's configuration in the Forefront " +
           "platform — " + ch.what + ".");
        return;
      ...
      /* ── THE ORDER A REVIEW RUNS IN (§266) ──────────────────────────
         Islam's answer, taken before it was built: the SMO. Asked through
         `lib/rules.js`, so the menu that draws the entry and the server that
         accepts the save are answering one question (§42).

         NOT GATED ON THE CYCLE LOCK, for `deckHide`'s reason: a locked cycle
      ...
      /* ── TAKING A DRAFT'S LOCK OFF (§301.6) ───────────────────────────
         The same two gates every reporting change has — the grant here, and
         a locked cycle — and deliberately NOT the own-lines refusal, which is
         the whole point: the people this exists for are the ones that
         refusal names. Submitting, parking, the note and the slides are
      ...
      /* §380: LOCKING YOUR OWN LINES. The key names the person, so the only
      ...
      /* §380: A LINE ENTERED BY ITS OWNER. The same three gates
