...
const REPORT = {
  ...
  /* §338: an action reports exactly as a milestone does — the same three
...
function collectUnit(key, su, iu, add, w) {
  ...
  if (!identified(su.items) || !identified(iu.items)) {
    ...
    Object.keys(sm).forEach(function (id) {
      ...
      /* ── §339: A PILLAR'S BREAKDOWN IS BOTH HALVES AT ONCE ──────────────
...
const FN_KNOWN = FN_SETUP.concat(UNIT_FOUNDATION,
  ["key", "items", "keyObjectives", "swot", "perf", "exec", "pend",
   ...
   "projects",
   /* §338: and the actions of a function that plans in objectives — in the
...
function fnOwnWork(sf2, iff, target, add) {
  ...
  /* §338: AND ITS ACTIONS, judged by the rules a milestone's row already gets
...
function ctxOfFn(w, subject) {
  ...
  holders.forEach(function (c) {
    ...
    /* §338: an action is a row of the holder's, and an id missing from this
