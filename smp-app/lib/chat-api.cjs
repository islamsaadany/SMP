/* smp-app/lib/chat-api.cjs — the frozen product's api/chat.js CARRIED ACROSS
   (spec 043 Phase G, §316.7) and edited in exactly ten ways for the shared
   schema. Every decision, every query and every sentence below this block is
   the frozen file's, byte for byte: §97's one conversation per person, §98's
   cadence and its five settings, §104's assistant and §125's narrated handoff,
   §231's push, §247's start, §282's unblocked read, §285's queue, §293's
   collection, §299's Ask. What changed is the PLUMBING, and nothing else:

     1. THE CLIENT IS HANDED IN, not connected here. `withTenant` opens the
        transaction and sets `app.tenant_id`, so every statement below runs
        under row-level security against the shared schema — the tables are
        the same tables and the SQL needs no schema qualifier, which is the
        whole point of §314's model. `P.connectFor`, `ensureReady`, `pg` and
        the `finally` that released the client all go with it.
     2. THE PERSON IS HANDED IN. The door resolved them (`personFor`), so
        `auth.getSession` goes; `me` keeps its shape — key, name, role,
        mustChange — and the two refusals it drove are made at the route.
     3. `send(res, code, obj)` RETURNS the answer instead of writing it. Every
        one of the sixty call sites is untouched: they all read
        `return send(res, …)`, so the function's own body is the only edit.
     4. `readBody` GOES — the route parses the JSON body and hands it over.
     5. `req` IS A SHIM carrying `headers.referer` and `method`, which is all
        this file ever asked of it (`platformHref`, and the GET default).
     6. THE EMAILS GO OUT INSIDE THE TRANSACTION, as they did inside the
        frozen request — stated rather than hidden, because a provider having
        a slow afternoon now holds a database connection as well as a request.
        §293's sweep REQUIRES a transaction (`pg_try_advisory_xact_lock` dies
        with it, §289), so moving the sends out would mean two transactions
        and a claim-then-send, which is exactly what that section refused.
     7. THE CLIENT'S SLUG IS HANDED IN. Three notifications open `/<client>`
        (§313.37: the worker used to open `/raya-trade` on every client), and
        the slug came off the connection's registry row; the route knows it,
        so it is a parameter.
    10. TWO `ON CONFLICT` TARGETS GAIN `tenant_id`, because the keys did.
        The second one found a fault in the SHARED SCHEMA rather than in this
        file: `push_subscriptions` was keyed `(tenant_id, person_key,
        endpoint)`, which is not "the key it has today with tenant_id in
        front of it" — one endpoint could then hold a row per person, so a
        device signed in to by somebody else would gain a second row instead
        of moving, and the previous person's notifications would keep
        arriving on it. Corrected in `db/schema.sql` with migration 003
        beside it (§316.7).
     9. `FROM org WHERE id = 1` LOSES ITS WHERE, in the four places it is
        asked. `org` is a SINGLETON: one row per tenant, keyed by `tenant_id`
        alone in the shared schema, where the frozen schema keyed it by a
        constant `id = 1` per client schema (data-model.md). The row-level
        policy is what narrows it now, which is the model — and the WHERE was
        not merely redundant, it named a column that does not exist, so the
        settings read threw and every chat request answered *Something went
        wrong*. Found by pressing it.
     8. TWO NESTED TRANSACTIONS BECOME SAVEPOINTS — the sweep's and the
        register read's. `withTenant` is already inside one and that is where
        `app.tenant_id` lives, so a `COMMIT` here would end the REQUEST's
        transaction and leave every tenant row invisible to everything after
        it. Both keep exactly what they were written for: the advisory lock
        still dies with the transaction (§289) and a failure still leaves the
        chat as it was rather than poisoning the poll that carried it.

   Nothing else moved. Where this file's own comments say "this endpoint" they
   still mean this file. */
...
const MAIL = require("./mail-html.cjs");
...
/* EDIT 4: `readBody` goes — the route parses the body and hands it over. */
...
/* EDIT 8 (the same one, one function up): a SAVEPOINT rather than a
   transaction of its own. The request already runs in one — `withTenant`'s,
   which is where `app.tenant_id` lives — so this block's `COMMIT` would end
   it and every later statement would read an empty world. What the frozen
   version was protecting is kept exactly: the two-second timeout is `SET
   LOCAL` and therefore scoped (§289.2 — as a bare `SET` it stayed on the
   pooled backend and cancelled somebody else's save), and a read that times
   out leaves the caller where it left it, with no register this second, which
   the caller already survives. Without the savepoint that timeout would abort
   the whole request, which is the opposite of what this exists for. */
async function readUnderLockTimeout(client, sql, params) {
  await client.query("SAVEPOINT reg");
  try {
    await client.query("SET LOCAL lock_timeout = '2s'");
    const r = await client.query(sql, params);
    await client.query("RELEASE SAVEPOINT reg");
...
/* EDIT 3: the answer is RETURNED. Every call site below still reads
   `return send(res, code, obj)` — untouched, all sixty of them — and the
   route turns what comes back into the response. `res` is carried as a
   parameter that is never read, so no line moves. */
...
async function chatSettings(client) {
  const r = await client.query("SELECT extra FROM org");
...
async function ensureThread(client, key, name) {
  await client.query(
    ...
    /* EDIT 10: the conflict target gains `tenant_id`, because the key did
       (schema.sql: "the key it has today with tenant_id in front of it"). */
    "ON CONFLICT (tenant_id, person_key) DO UPDATE SET person_name = COALESCE(EXCLUDED.person_name, chat_threads.person_name)",
    [key, name || null]);
}
...
async function assistantAnswer(client, me, question, given) {
  ...
  try {
    ...
    const org = (await client.query("SELECT extra FROM org")).rows[0] || {};
...
async function mailShape(client, req) {
  const r = (await client.query("SELECT org_name, extra FROM org")).rows[0] || {};
...
async function sweep(client, cfg, req) {
  ...
  LAST_SWEEP = now;
  /* EDIT 8: A SAVEPOINT, WHERE THIS OPENED ITS OWN TRANSACTION. `withTenant`
     is already inside one — that is where `app.tenant_id` lives, and it is
     transaction-local — so a `BEGIN` here is a no-op with a warning and the
     `COMMIT` would end the REQUEST's transaction, taking the tenant setting
     with it and leaving every row invisible to everything after (§314's
     policy compares against null). A savepoint keeps both properties this
     block was written for: `pg_try_advisory_xact_lock` still dies with the
     transaction (§289, and it is now the request's, which is longer and
     strictly safer), and a sweep that throws still leaves the chat exactly as
     it was rather than poisoning the request it rode in on — a failed
     statement aborts a whole transaction in Postgres, so without the
     savepoint one bad collection would refuse the poll that carried it. */
  try {
    await client.query("SAVEPOINT sweep");
    const got = (await client.query("SELECT pg_try_advisory_xact_lock($1) AS ok",
                                    [SWEEP_LOCK])).rows[0];
    if (!got || !got.ok) { await client.query("RELEASE SAVEPOINT sweep"); return; }
    await collectForOffice(client, cfg, req);
    await collectForPeople(client, cfg, req);
    await client.query("RELEASE SAVEPOINT sweep");
  } catch (e) {
    try { await client.query("ROLLBACK TO SAVEPOINT sweep"); } catch (e2) { /* the pool will drop it */ }
...
/* EDIT 1, 2 and 5: the client, the person and the body are handed in. The
   route holds the door and the transaction; this file holds the chat. */
module.exports = async function handler(client, me, body, req, slug) {
  const res = null;
  try {
    ...
    /* The two refusals this line drove — no session, and a password still
       temporary (§43.2) — are made at the route, in front of every endpoint
    ...
    if (action === "pushOn") {
      ...
      await client.query(
        ...
        "ON CONFLICT (tenant_id, endpoint) DO UPDATE SET person_key = EXCLUDED.person_key, " +
    ...
    if (action === "assistantTest") {
      ...
      if (kb && assistant.configured()) {
        const orgT = (await client.query("SELECT extra FROM org")).rows[0] || {};
    ...
    /* THE TRANSACTION IS THE CALLER'S, so a throw here has to reach it:
       `withTenant` rolls back on one, and swallowing it would COMMIT half a
       send. The route turns it into the same sentence this file used to
       write itself, and the runtime log keeps the cause (§43). */
    throw e;
  }
};
