...
import { oneItem, countDownload, safeFileName, type Kind } from "./library.ts";
...
export async function libraryFile(tenantId: string, kind: Kind, id: string, _url: string): Promise<Response> {
  ...
  const item = await withTenant(tenantId, (c) => oneItem(c, kind, id, true));
