/* smp-app/lib/mail-api.cjs — the frozen product's api/mail.js CARRIED ACROSS
   (spec 043 Phase G, §316.7) and edited in the same six ways `chat-api.cjs`
   is: the client and the person are handed in, `send()` RETURNS its answer,
   `readBody` goes, `req` is a shim carrying `headers` and `method`, and the
   throw reaches `withTenant` so a half-written send rolls back. Every
   decision below is the frozen file's, byte for byte — §72's one place the
   credential is read, §74's per-recipient loop, §135's greeting filled from
   the STORED register, §146's test copy on the record, §144's landing.

   ONE THING MORE: `readState` came from `lib/state-io.js`, whose reader is
   carried here as `graph-io.cjs` (§314's row-addressed writer lives beside
   it). The audience is resolved from the graph exactly as before. */
/* ── SENDING MAIL (§72) ───────────────────────────────────────────────────
...
const { readState } = require("./graph-io.cjs");

const RESEND = mailer.RESEND;

/* The answer is RETURNED; every `return send(res, …)` below is untouched. */
function send(res, code, obj) {
  return { code: code, body: obj };
}
/* `readBody` goes — the route parses the body and hands it over. */
...
module.exports = async function handler(client, me, body, req) {
  const res = null;
  try {
    ...
    /* No session, and a password still temporary, are refused at the route —
       in front of every endpoint rather than inside one of them. */
    ...
    /* THE TRANSACTION IS THE CALLER'S — `withTenant` rolls back on a throw,
       and swallowing it here would COMMIT half a send. */
    throw e;
  }
};
