...
/* THE TABLE IS THE SCHEMA'S NOW (spec 043 §316.7, the one edit this carried
   copy makes). The frozen module created it on first use, which was right
   where the endpoint connected as the schema's owner; the shared schema is
   reached as `smp_app`, which holds DML on every table and no CREATE at all —
   and `CREATE TABLE IF NOT EXISTS` is refused for want of the privilege
   before it ever looks at whether the table is there (measured: *permission
   denied for schema public*). `push_keys` is created by `db/schema.sql` as a
   PLATFORM table, one VAPID pair per deployment and outside every tenant's
   rows, which is what §231 wanted and what the frozen comment above says. */
...
async function sendTo(client, subs, payload) {
  ...
  const gone = [];
  /* ── THE REFUSAL'S OWN WORDS (§282.2) ─────────────────────────────
     THIS WAS COUNTED AND THROWN AWAY, and it is why four rounds of fixes
     changed nothing: measured against a stand-in push service, a mismatched
     key (403), a dead registration (400) and an oversized payload (413) all
     came back as `failed: 1` and printed the same sentence — "The push
     service would not take it." Three different errands, one shrug.

     §124's fault at the far end of the chain: that section is about presence
     reported as proof, this is failure reported without its cause, and both
     leave somebody looking at everything because nothing named one thing.
     The service says exactly what is wrong; it is now carried back. */
  let why = null;
  ...
  await Promise.all(subs.map(async function (s) {
    try {
      ...
      if (code === 404 || code === 410) { gone.push(s.endpoint); dropped++; }
        ...
        failed++;
        /* THE FIRST ONE, AND ONLY THE FIRST — a person with three devices on
           one broken key would otherwise repeat one sentence three times.
           The body is what carries the meaning ("VAPID credentials do not
           match the subscription"), so it is kept whole and merely bounded,
           and NOTHING HERE IS A SECRET: it is the service's answer to us, and
           the private key never appears in it (§72's rule for the mailer). */
        if (!why) why = refusalWords(err, s.endpoint);
      }
    }
  }));
  ...
  return { sent: sent, dropped: dropped, failed: failed, why: why };
}

/* WHICH SERVICE, AND WHAT IT SAID (§282.2).

   THE SERVICE IS NAMED BECAUSE "IT DOES NOT WORK" IS A DIFFERENT ERRAND ON
   EACH ONE: Apple needs the platform on a home screen before it will deliver
   at all, Google needs nothing, and a person reporting silence on an iPhone
   and a person reporting it on a laptop are not reporting the same thing.
   The endpoint's HOST says which, and the host is all that is taken from it —
   the rest of the endpoint IS the address of somebody's device and has no
   business in a diagnostic. */
function serviceOf(endpoint) {
  let host = "";
  try { host = new URL(String(endpoint)).host; } catch (e) { return "the push service"; }
  if (/\.googleapis\.com$/.test(host)) return "Google (Chrome, Edge, Android)";
  if (/push\.apple\.com$/.test(host)) return "Apple (Safari, iPhone, iPad)";
  if (/mozilla\.com$/.test(host)) return "Mozilla (Firefox)";
  if (/notify\.windows\.com$/.test(host)) return "Microsoft";
  return host;
}

function refusalWords(err, endpoint) {
  const code = err && err.statusCode;
  const raw = String((err && (err.body || err.message)) || "").trim().slice(0, 300);
  const svc = serviceOf(endpoint);
  /* THE ONE CAUSE WORTH NAMING IN OUR OWN WORDS, because it is the one the
     office can do something about and the one the service describes in terms
     nobody outside this file would recognise: the device registered against a
     key this platform is no longer sending with, so every send is refused for
     ever and nothing about it ever heals (§282.3 re-registers it). */
  if (code === 403 || /vapid|credential|mismatch|unauthorized/i.test(raw)) {
    return svc + " refused it: this device was registered with a different " +
           "key from the one the platform is sending with, so it can never " +
           "receive anything until it registers again." +
           (raw ? " It said: " + raw : "") +
           " (" + (code || "no status") + ")";
  }
  return svc + " would not take it" +
         (code ? " (" + code + ")" : "") + (raw ? ": " + raw : ".");
}
...
/* ── WHERE THE CHAIN STOPS (§231.6) ───────────────────────────────────
   §123's argument, for the other silent feature: "it is not working" sends
   somebody to look at everything, and naming the step sends them to one page.
   Every link here fails invisibly by design — a device that never registered,
   a key that was never minted, a library that did not load, a push service
   that refused — and until this existed neither the office nor I could tell
   them apart.

   IT REPORTS, IT NEVER REPAIRS. Reading is safe to offer to anybody who can
   open the settings; changing things from a diagnostic is how a diagnostic
   becomes the thing that broke it. */
async function health(client, personKey) {
  const out = { library: !!lib(), libraryWhy: webpushWhy || null,
                key: "", keyFrom: null, devices: 0, subject: subject() };
  try {
    const k = await keys(client);
    out.key = k.publicKey; out.keyFrom = k.from;
  } catch (e) { out.keyWhy = (e && e.message) || "no key pair"; }
  try {
    out.devices = ((await client.query(
      "SELECT count(*)::int AS n FROM push_subscriptions WHERE person_key = $1",
      [personKey])).rows[0] || {}).n | 0;
  } catch (e) { out.devicesWhy = (e && e.message) || "could not be read"; }
  return out;
}

module.exports = { publicKey, sendTo, subsOf, officeSubs, health,
                   serviceOf };
