...
(function (root, factory) {
  ...
  var PAGES = [
    ...
    /* HISTORY (§262). `area:"always"` with the real gate in the page def and
  ...
  function personRoles(w, p) {
    ...
    /* AND A FUNCTION'S OWN PROJECTS (§334, closing §326). That section gave a
  ...
  /* ── A CAPABILITY'S ACCESS IS ITS HOLDER'S (§334) ────────────────────
  ...
  /* ── 4 · THE STRATEGY TAB IS AUTHORED BY THE OFFICE (§94) ─────────
     ...
     IT FAILS CLOSED FOR EVERY PEN. `penBtn()` and `secActs()` in the platform
     ...
     remembers (§42's rule, on the screen instead of the server). (`editBar()`
     was the third and is gone: §268 moved a supporting function's Overview
     control onto the section line, where `secActs()` draws it.)
...
var GAP_OPTIONAL = { tactic: ["collaborators"],
  ...
  /* THE OTHER DIRECTION: a month count written the way the cycle writes it
     (§307). `whenMonths` reads every shape the product has ever stored; this
     writes the ONE shape it now produces for a cycle's own dates — a short
     month and a FOUR-DIGIT year.

     FOUR DIGITS IS NOT A STYLE. `cycleYear()` scrapes a four-digit year out of
     the cycle's `to`, `name` and `due`, and since §307 `to` is the review point
     — so a two-digit "Jun 27" there would take `to` out of that scrape and land
     §239.3's fault (a cycle whose year cannot be read stops prorating
     entirely) on the very field this change made load-bearing. A milestone's
     picker still writes two digits, which is the shape those plans carry.

     Shared, because the browser draws it and the heal that merges the two
     fields writes it (§42). */
  function monthLabel(t) {
    if (t == null) return "";
    return MONTH_NAMES[((t % 12) + 12) % 12] + " " + Math.floor(t / 12);
  }
  ...
  /* ── THE COMPILE RULES, NAMED ONCE (§276) ────────────────────────────
     Islam: *"we need a compilation type that prorate to integrs only .. if we
     have a target of 2 shops to open in the year so in the 8th month that
     proration asks for 1.3 stores which is not feasible."*

     `compile` says what kind of number a target is, and the list was spelled
     SEVEN times across the pen, the plan builder and the workbook's
     validation ranges — which is how a fourth value gets added to six of them.
     It is one list now, in the order the picker shows it: the two that add up
     side by side, then the two that do not.

     WHICH RULES PRORATE, and the plan already answers it (§239): `Sum` adds up
     across the period, so six months of accumulation against twelve months of
     target is the wrong comparison. `Count` is Sum for things finished one at a
     time — shops opened, systems consolidated — and rounds the due figure DOWN
     to a whole one, because a shop is not owed until its whole share of the
     year has passed. `Latest` is a rate at a point in time and `Average` is
     already normalised; neither has anything to prorate.

     A FOURTH RULE, NOT A CHANGE TO SUM. Making Sum round whenever the target
     looks whole would silently move stored scores on every existing Sum row
     with an integer target (the demo alone has "Systems consolidated 6" and
     "Cycles reviewed on time 4"). A new value means nothing anybody has
     already planned moves, and choosing it is a plan decision (§99.8). */
  var COMPILES = ["Sum", "Count", "Latest", "Average"];
  function compileKey(c) { return String(c == null ? "" : c).trim().toLowerCase(); }
  function prorates(compile) {
    var c = compileKey(compile);
  ...
  /* ── A YES OR A NO THAT CAN BE UNDER WAY (§300) ────────────────────
     Islam: *"sometimes we have a directly y/n situation like getting
     profitable so nothing to be in progress there but some y/n can take in
     the in progress like an agrement we are working on"* — and, of the three
     shapes put to him, C: one kind of Y/N row, and the REPORTING control
     gains the middle answer.

     THE PRODUCT HAD ALREADY ANSWERED THIS ONCE, ONE COLUMN OVER. §104 deleted
     a deliverable's `kind` — the plan's old choice between "delivered or not"
     and "a percentage" — because Not started / In progress with a per-cent /
     Delivered serves both, and a row that is genuinely done-or-not simply
     never touches the middle. So this is that model on a measure, not a
     second one beside it (§53.5), down to the words: **Not started** and
     **Done** are what a tactic's own Status column has said since the model
     was written, one cell to the left of the one being answered. Islam,
     correcting the first drawing: *"there is no not yet answer. it's either
     Done, In progress, Didn't Start."*

     ONE FIELD, BECAUSE THE ROW HAS ONE. A milestone keeps `status` and `pct`
     in two columns; a measure has `actual` and nothing else, so a second
     field would mean a schema change, a workbook column and a round trip that
     can lose it (§22) — for a fact that is one answer. The answer is stored
     whole, in the words above: `Not started`, `In progress`, `In progress 60`,
     `Done`. `ynState` is the ONE reader of that string and `ynJoin` the one
     writer, so the parsing lives in one place rather than at every call site.

     AND THE OLD SPELLINGS GO ON READING. `Yes` and `No` are every Y/N figure
     any tenant already holds: they answer `done` and `todo`, score 100 and 0
     exactly as they did (§30.2 — nothing stored moves, nothing is migrated),
     and are shown in the new words. A value that is neither — a number typed
     before this existed — is NOT an answer rather than a nought (§35).

     THE PARTIAL IS JUDGED AGAINST THE ROW'S OWN WINDOW, and Islam is the
     reason it is: *"a progress of 60% in Q2 while the action reuqire Q2 & 3
     doesn't score 60% that's 60% against due of 50%."* Right, and my first
     answer had generalised from the wrong case — a tactic STATES its window
     in its quarters (§250's `tacticShare`), so there is no glide path for the
     platform to invent, and 60 against a half-elapsed window is 120: ahead.
     Where nothing states a window — a pillar's key measure, a unit's key
     objective — the share is absent and the partial reads as ITSELF, because
     inventing a straight line to December would mark a December commitment
     behind all year (§239's own refusal to prorate `Latest` and `Average`).

     DONE DOES NOT PRORATE, AND THAT IS HIS CALL WITH ITS COST STATED: *"yes
     delivers the 100% and that's it. delivering early of an action doesn't
     indicate a higher % of achievement."* So a row at 60% of a half-elapsed
     window reads 120 and the same row, finished, reads 100 — finishing lowers
     the number on a row that was ahead, which is the price of a yes meaning
     one thing whenever it arrives.

     NOTHING DUE YET IS NOT NOUGHT. With a window that has not begun there is
  ...
  function ynState(v) {
    ...
    /* "In progress" alone is a row halfway through a sentence: said, and not
       finished saying. The number after it is optional HERE and required by
       the gate (§104.10's own separation of `given` from `pending`). */
    if (low.indexOf("in progress") === 0)
      return { status: "wip", pct: ynPct(s.slice("in progress".length)) };
    return { status: "", pct: null };
  }
  function ynJoin(status, pct) {
    if (status === "done") return YN_DONE;
    if (status === "todo") return YN_TODO;
    if (status !== "wip") return "";
    var p = ynPct(pct);
    return p == null ? YN_WIP : YN_WIP + " " + p;
  }
  /* SAID, AND SAID COMPLETELY. An In progress with no per-cent is not an
     answer — the tally does not count it, the page marks it and Submit waits
     for it — which is exactly what a milestone in the same state already does
     (§104.10). One predicate, asked by the screen and by the count. */
  function ynAnswered(v) {
    var st = ynState(v);
    return st.status === "todo" || st.status === "done" ||
           (st.status === "wip" && st.pct != null);
  }
  /* WHAT IS DRAWN, wherever a Y/N figure is READ — the pane, the projector,
     the workbook's own reader. The stored string is the product's, so a row
     holding a tenant's old `Yes` reads in today's words without anything
  ...
  function ynScore(v, share) {
    var st = ynState(v);
    if (st.status === "done") return 100;
    if (st.status === "todo") return 0;
    if (st.status !== "wip" || st.pct == null) return null;
    /* An absent share is "this row names no window" and reads as itself; a
       share of nought is "its window has not begun", which owes nothing and
       so cannot be scored against. The two are different answers and the
       difference is the whole of §300's second decision. */
    if (share == null) return st.pct;
    if (!(share > 0)) return null;
    return Math.max(0, Math.min(150, Math.round(st.pct / share)));
  }
  /* §257's own reader, kept: two callers outside this module ask whether a
     stored figure is a plain yes or no, and both mean the two ENDS. */
  function ynAnswer(v) {
    var st = ynState(v);
    return st.status === "done" ? true : st.status === "todo" ? false : null;
  }
  ...
  /* §276 CAME IN ON THE SAME MERGE AS THIS, AND IT IS A FOURTH RULE.
     `Count` prorates exactly as `Sum` does and is then owed in whole ones —
     so twelve months are added up and floored, which is the same relationship
     Count has to Sum everywhere else (§53.5). Left out of this list it would
     have been the quietest possible hole: the office fills twelve boxes on a
     Count row, nothing happens, and the drawer says the row has no compile
     rule when it plainly has one. Read from `COMPILES` rather than listed
     again, or a fifth rule added later is in one list and not the other. */
  function monthlyCompile(row) {
    var c = compileKey(row && row.compile);
    for (var i = 0; i < COMPILES.length; i++)
      if (compileKey(COMPILES[i]) === c) return c;
    return "";
  }
  ...
  function monthlyDue(row, months) {
    ...
    if (c === "average") return t / n;
    /* §276's own rounding, on the months rather than on the share: a count is
       owed in whole ones, and the epsilon is not decoration — twelve typed
       decimals can sum to 1.9999999999999998, and a floor taken on that owes
       one fewer than the arithmetic means. */
    return c === "count" ? Math.floor(t + 1e-9) : t;
  }
  ...
  /* ── THE SAME QUESTION, ONE LIST OVER (§272) ──────────────────────
     `gapMissing` answers *what does this row OWE*; this answers *what may a
     filler write here that holds nothing*, which is `GAP_FILLABLE` and is
     the list §223's door reads. It was written out inline in `gapMap`'s
     fillable branch and needed for a second and a third reader the moment
     the empty register gained a count and a rail mark — three copies of one
     predicate is how a band and a rail come to disagree about one pillar
     (§53.5, §116.2: the count and the walk are ONE list).

     NO PENDING CLAUSE, deliberately, and that is not an omission: a mark is
     written only for a value the platform can use (§249.2), so a marked
  ...
  /* ── IS THERE ANYTHING TO TOUR (spec 042 §6.2) ───────────────────
     §107's onboarding tour walked a FILLED platform by switching the page to
     the baked worked example. That switch is gone with the Demo-data button
     (spec 042 §6.1): the example is a client of its own now, so the tour runs
     on the person's own plan — and a tour of an empty screen teaches nothing,
     so it is not offered until there is one.

     THE TEST IS THE SAME ONE THE STORIES NEED: a place to tell it on that
     holds at least one pillar or capability AND one key objective, which is
     exactly what the steps point at. Asked of a TARGET rather than of the
     tenant, because the story is told on a unit or a function — a group with
     ten filled units and an empty one still has nothing to show somebody who
     works in the empty one (§61's trap, one surface out).

     Here rather than in tour.js because a rule with one caller today is still
     a rule, and the deck, the check and any later "is this client ready"
     question must not each answer it their own way (constitution IX). */
  function tourReady(w, target) {
    if (!w || !target) return false;
    var u = unitLikeOf(w, target);
    if (!u) return false;
    var items = (u.items || []).length > 0;
    var kos = (u.keyObjectives || []).length > 0;
    return items && kos;
  }

  /* A unit key or `fn:<key>`, resolved in ONE place (§59's rule). */
  function unitLikeOf(w, target) {
    if (!w || !target || target === "group") return null;
    if (String(target).indexOf("fn:") === 0) {
      var f = (w.functions || {})[String(target).slice(3)];
      return f || null;
    }
  ...
  /* §270: AN UNMAPPED KEY PASSES THROUGH. It used to fall back to `k_proj`,
     which was harmless while the only callers were three literals here and in
     the authoriser, all of them unit-side strategy keys — and unsafe the
     moment anything asked this of a key that is not one, because every
     unrecognised page would have become a function's Projects. The screen now
     resolves EVERY access key through this on its way to `mayAuthorPage()`
     (which is what closes §217's remaining half), so it is asked about
     `k_found`, `u_perf`, `c_people` and the rest as a matter of course.
  ...
  function strategyPageOf(target, unitPage) {
    /* §334: A CAPABILITY TAKES THE FUNCTION'S PAGES, because it IS drawn by
  ...
  var CHAT_DEFAULTS = Object.freeze({
    ...
    assistant: false,
    /* ── AND THE OFFICE'S OWN ASK, ITS OWN SWITCH (§299) ───────────
       Islam: *"in the settings the enablement of the assistant for the smo
       and the users should be splitted"*, and *"a new switch for the smo."*

       A SEPARATE KEY RATHER THAN A WIDER READING OF THE ONE ABOVE, and that
       is the whole of why it is here: `assistant` means *answer people before
       the office does*, and every tenant that has already turned it on chose
       exactly that. Reading it as "and give the office an Ask box too" would
       switch a new capability on for all of them without anybody being asked
       (§30.2's rule about a stored key keeping its meaning, from the other
       side).

       OFF, read the same way round as the three above it — only an explicit
       `true` — so a deployment that upgrades gets exactly the chat it had.

       IT ANSWERS FROM THE SAME CORPUS AND STARTS NOTHING. For everybody else
       the assistant is the first line of a conversation a person can take
       over; for the office there is nobody to take over, because they ARE the
       people who would — so an Ask never joins the waiting list, never counts
       on the badge and is never chased by email. The guard is on the server,
       in `ask`, for §42's reason: with this off there is no control on screen
       to hide, so the refusal IS the setting. */
    ask: false,
    ...
    popup: false,
    rep: "",
    /* ── HOW LONG THE PLATFORM COLLECTS BEFORE IT EMAILS (§169, §293) ──
       WAS "how long somebody counts as here" (§169), AND ISLAM RETIRED THAT
       QUESTION: *"I believe the 3 min is not relevant now and we can adjust
       this setting to be the one which identifies the time away before
       sending the email, which is set for now in our case a 10 min."*

       §293 stopped presence deciding anything about email — *"even if I'm at
       my desk if the smo don't reply in 10 min the email should come ...
       sometimes people might be at their desk but not focusing"* — so the
       away threshold had nothing left to decide, and rather than leave a
       number that governs nothing beside a new one that does (which is
       exactly the duplication he objected to), the SAME key changes what it
       means. One box, one number, relabelled *Email after*.

       AND IT SUPERSEDES §283's `chase`. That section built the other half of
       this — a reply to somebody who LOOKED PRESENT, still unread N minutes
       later, emailed on the way past — which is the same feature reached from
       the other side, and it left two numbers on one panel: Islam, of exactly
       that shape, *"I'm confused between the send email after 3 min and the
       send after 60 min is that a duplication?"* One rule, one number, and
       presence deciding neither.

       THE KEY DOES NOT MOVE (§30.2). A tenant that typed a number keeps it;
       what changes is the SHIPPED DEFAULT, which is what an untouched tenant
       reads — 3 was a presence threshold and 10 is a collecting time, and a
       tenant that never set one should get the new answer rather than the old
       number under a new name.

       THE SCREEN'S OWN "they have the platform open" IS NOT THIS NUMBER
       (`CHAT_HERE_MIN` below). That sentence is about this moment and is only
       ever a description; making it follow a collecting time would have it
       call somebody present for ten minutes after they closed the tab. */
    away: 10
  });
  var CHAT_PROMISE = "Usually answers the same day";
  /* WHAT THE SCREEN MEANS BY "they have the platform open" (§293). Not a
     setting: it describes this moment, and the one number a tenant sets is
     how long the platform COLLECTS before emailing, which is a different
     fact and a longer one. A shut panel checks in every 180 seconds (§98's
     idle beat), so this cannot go below four without calling somebody away
     while they sit at their desk. */
  var CHAT_HERE_MIN = 4;
  ...
  function chatCfg(raw) {
    ...
    return {
      ...
      /* READ THE SAME WAY AS `away` (§169): absent is not nought, because
         `Number(null)` and `Number("")` are both 0 and both finite — a clamp
         alone would answer ONE MINUTE for every tenant that has never set
         this, which is exactly the fault that section recorded. */
      away:      (function (v) {
  ...
  /* ── IS THERE ANYTHING OLD ENOUGH TO EMAIL? (§293) ────────────────
     ONE DEFINITION, THREE ASKERS: the sweep that emails the office, the sweep
     that emails a person, and the line that tells the office what will happen
     after they press Send — because a screen that predicts and a server that
     decides separately is the drift this module exists to prevent (§97.5).

     `oldest` is when the OLDEST thing nobody has been emailed about arrived.
     Nothing waiting is not "not yet due", it is nothing to send — the two
     read the same from a boolean and mean opposite things, so the empty case
     answers false and the caller never builds an email with nothing in it
     (§45.2 in an inbox).

     A TIME THAT WILL NOT READ ANSWERS YES, because the two failures are not
     equal: one email early is a nuisance, and a question nobody is told about
     is the whole feature not working (§35). */
  function chatCollectDue(oldest, now, minutes) {
    if (oldest === null || oldest === undefined || oldest === "") return false;
    var t = (oldest instanceof Date) ? oldest.getTime()
          : (typeof oldest === "number") ? oldest
          : Date.parse(String(oldest));
    var n = (now instanceof Date) ? now.getTime()
          : (typeof now === "number" && isFinite(now)) ? now : Date.now();
    if (!isFinite(t)) return true;
    var m = chatCfg({ away: minutes }).away;
    return (n - t) >= m * 60000;
  }
  ...
  /* ── THE SAME QUESTION, NEVER A SIMILAR ONE (§299) ──────────────────
     Case, the space around it and a trailing question mark are one question
     written twice; anything looser is the platform deciding that "how do I
     close the cycle" and "when does the cycle close" are one errand, in front
     of the office and wrong often enough that the counts stop being read.
     Refused deliberately when this was drawn.

     IT IS SHARED BECAUSE BOTH SIDES ASK IT (§42): the server groups the list
     on it, and the Knowledge base page asks whether an answer has already been
     written for a row — matching the question it minted the entry from. Two
     copies would answer differently the first time either is corrected, and
     the disagreement would read as a row that will not stop asking. */
  function askKey(q) {
  ...
  /* ── A VIDEO IN THE REVIEW (§261) ─────────────────────────────────────
     ...
     the presentation how do you think we can do it without overloading the
     data base with videos?"* — and then, of the two ways he weighed,
     *"upload and a link to paste"*, 3 a subject, kept until the office clears
     them, and a room that is connected.

     THE QUESTION WAS ABOUT QUALITY AND THE ANSWER IS ABOUT PLACE. A picture
     is shrunk in the browser and stored in the state graph (§50), and that
     works because the graph is 297KB and a picture is 150–300KB of it. A
     two-minute clip at 720p is 20–40MB — about a hundred times the entire
     tenant — and every person downloads the whole graph on every sign-in, so
     squeezing moves the number and not the order of magnitude. It does not
     go in the graph at ANY quality. What goes in the graph is a POINTER.

     THE PLACES A CLIP MAY PLAY FROM ARE NAMED, and that is Islam's own choice
     between two put to him with the cost of each: naming them means a clip on
     a fifth service opens in a new tab until the office adds that service;
     not naming them means the page would have to admit media and frames from
     anywhere, which is an open door for an injected script to frame a false
     sign-in inside the platform's own chrome. He took the narrower one.

     ONE READER, ASKED BY EVERYBODY. The paste box says at once whether a link
     will play (§32, §171 — a person finds out at their desk, never in the
     meeting room), the deck draws from the same answer, and the server judges
     an incoming link with it. Three copies of "is this a video we can play"
     is §42's drift with a projector on the end of it. */
  var VIDEO_MAX_BYTES = 50 * 1024 * 1024;   /* his ceiling, agreed */
  var VIDEO_MAX_SECS = 120;
  var VIDEO_PER_SUBJECT = 3;                /* per unit AND per function, per cycle */

  /* A clip we hold ourselves. The bytes are in the blob store and the graph
     keeps this path; the platform's own endpoint checks the session and sends
     the browser on, so no address that outlives a sign-in is ever written
     into a slide. */
  var VIDEO_STORE = "blob";

  /* WHAT COUNTS AS A DIRECT FILE. Extensions only, because this is the one
     test that can be made about a URL without fetching it — and fetching it
     to find out would be the server going wherever a person typed (§71). */
  var VIDEO_FILE_RE = /\.(mp4|m4v|webm|ogv|mov)(\?|#|$)/i;

  /* The services the platform knows how to play ON THE SLIDE, each with the
     address its player actually lives at. A share link is a PAGE, not a
     video: dropping `youtube.com/watch?v=…` into a player shows nothing at
     all, which is why every one of these carries a rewrite rather than only
     a hostname. SharePoint is passed through as given — its own viewer
     renders in place for somebody signed in to SharePoint, and if they are
     not, SharePoint says so in its own words, which is better than us
     guessing at a rewrite that changes with their tenant's settings. */
  var VIDEO_SERVICES = [
    { key: "youtube", name: "YouTube",
      host: /(^|\.)(youtube\.com|youtube-nocookie\.com|youtu\.be)$/i,
      embed: function (u) {
        var id = "";
        if (/youtu\.be$/i.test(u.host)) id = u.path.split("/")[1] || "";
        else if (/^\/embed\//.test(u.path)) id = u.path.split("/")[2] || "";
        else if (/^\/shorts\//.test(u.path)) id = u.path.split("/")[2] || "";
        else id = param(u.query, "v");
        return /^[\w-]{6,}$/.test(id)
    ...
    { key: "vimeo", name: "Vimeo",
      host: /(^|\.)(vimeo\.com)$/i,
      embed: function (u) {
        var m = u.path.match(/\/(\d{6,})/);
        return m ? "https://player.vimeo.com/video/" + m[1] : "";
      } },
    { key: "drive", name: "Google Drive",
      host: /(^|\.)(drive\.google\.com)$/i,
      embed: function (u) {
        var m = u.path.match(/\/file\/d\/([\w-]+)/);
        return m ? "https://drive.google.com/file/d/" + m[1] + "/preview" : "";
      } },
    { key: "sharepoint", name: "SharePoint",
      host: /(^|\.)(sharepoint\.com)$/i,
      embed: function (u) { return u.href; } }
  ];

  function param(query, key) {
    var out = "";
    String(query || "").split("&").forEach(function (bit) {
      var i = bit.indexOf("=");
      if (i > 0 && decodeURIComponent(bit.slice(0, i)) === key) {
        out = decodeURIComponent(bit.slice(i + 1).replace(/\+/g, " "));
      }
    });
    return out;
  }

  /* Split without `new URL`, which throws on anything malformed and is one
     more thing to guard; every part this needs is in the string. */
  function urlParts(href) {
    var m = String(href).match(/^https?:\/\/([^\/?#]+)([^?#]*)\??([^#]*)/i);
    if (!m) return null;
    return { href: href, host: m[1].replace(/:\d+$/, ""), path: m[2] || "/",
             query: m[3] || "" };
  }

  /* WHAT A PASTED LINK IS, in one answer:
       kind "embed" — a service we know, `play` is its player's address
       kind "file"  — a video file we can play directly
       kind "away"  — accepted, and it opens in a new tab rather than playing
       kind ""      — not a web address at all
     `hosts` is the office's own list from Setup › Video storage: a company
     that keeps its videos on its own server adds that server once and its
     links play on the slide from then on. */
  function videoLink(raw, hosts) {
    var href = webUrl(raw);
    if (!href) return { kind: "", url: "", play: "", service: "" };
    var u = urlParts(href);
    if (!u) return { kind: "", url: "", play: "", service: "" };

    for (var i = 0; i < VIDEO_SERVICES.length; i++) {
      var s = VIDEO_SERVICES[i];
      if (s.host.test(u.host)) {
        var play = s.embed(u);
        /* A YouTube address with no video in it is still YouTube, and saying
           "it will open in a new tab" is the honest answer rather than
           refusing a link somebody plainly meant. */
        return play
          ? { kind: "embed", url: href, play: play, service: s.name }
          : { kind: "away", url: href, play: "", service: s.name };
      }
    }
    if (VIDEO_FILE_RE.test(u.path) || ownHost(u.host, hosts)) {
      return { kind: "file", url: href, play: href, service: "" };
    }
    return { kind: "away", url: href, play: "", service: "" };
  }

  /* An exact host, or any name under it. Never a substring test: "evil-
     raya.com".indexOf("raya.com") is a match, and that is somebody else's
     server playing inside our page. */
  function ownHost(host, hosts) {
    host = String(host || "").toLowerCase();
    return (hosts || []).some(function (h) {
      h = String(h || "").toLowerCase().replace(/^\*?\./, "").trim();
      return !!h && (host === h || host.slice(-(h.length + 1)) === "." + h);
    });
  }

  /* Every host the page must be allowed to reach, derived from the services
     above rather than typed a second time into the policy — a list spelled
     twice is §234's fault, and here the second spelling is the one that
     decides whether anything plays at all. */
  var VIDEO_EMBED_HOSTS = [
  ...
  /* A subject's clips, and what stands between one more and the ceiling.
     Counted from the SLIDES rather than from a tally kept beside them: a
     number somebody has to remember to decrement is a number that drifts. */
  /* A clip is EITHER ours or somebody's link, and the field says which: `path`
     is a file in our own store, `url` is an address the office pasted. Nothing
     stores what KIND of link it is — that is derived at the moment of drawing
     (`videoLink`), so the day the office adds their own host, every link
     already pasted for that host starts playing on the slide (§42: one
     reader, asked when the answer is needed). */
  function videoHeld(sl) {
    var v = sl && sl.vid;
    return !!(v && (v.path || v.url));
  }
  function videoSlides(list) {
    return (list || []).filter(videoHeld);
  }
  function videoRoom(list) {
    return Math.max(0, VIDEO_PER_SUBJECT - videoSlides(list).length);
  }
  ...
  /* ── THE MASTER PRESENTATION'S RUNNING ORDER (§266) ─────────────────
     Islam: *"give an option for the SMO from the presentation list to do
     master presentation which is a flow of presentations in a flow and he is
     just asked the flow of the units and functions who will present."*

     IT IS A LIST OF SUBJECTS AND NOTHING ELSE. Every deck in the flow is
     assembled by the reader that already exists (`deckHtmlFor`), so what is
     stored is WHO presents and in what order — never a slide, never a copy of
     a deck. That is the same argument §50 makes about picture slides: a
     stored deck is the exported deck the whole feature exists to avoid.

     IT LASTS, so it does not ride `REVIEW`, which the cycle clears (§50,
     §115): the order a board meeting runs in is agreed once and used every
     quarter. It rides the GROUP, whose `extra` already carries `sets`,
     `naming` and `focusOff` — so no migration — and is stored as an ABSENCE
     (§50.6): a tenant that has never opened the picker and one that set an
     order and cleared it again are the same bytes.

     THE FIELD IS NAMED ONCE and `lib/authorize.js` classifies it by this
     constant, never by a literal of its own — §234's fault is what it costs
     when the two lists that must agree are kept apart (§256's own note).

     WHO MAY OPEN IT IS A RULE, NOT A MATRIX CELL, and it is Islam's answer to
     the question asked before it was built: the SMO. `mayDownloadPlan` above
     is narrowed the same way and for the same reason — the entry sits in the
     Presentation menu beside it, and a menu that offers what the press then
     refuses is §42's drift with a projector on the end of it. */
  var MASTER_FLOW = "masterFlow";
  /* ── THE PLANNING PERIOD (§308) ────────────────────────────────────
     The period a target belongs to — normally a year, and shorter where a
     plan started mid-year. Islam: *"the main planning cycle is not a year
     maybe 6 month but the review cycle might have the same start date but
     will have a closer end date."*

     TWO NAMES IN ONE PLACE, for §234's reason and §259.2's: `lib/authorize.js`
     both CLASSIFIES these keys and lists them among the group's known ones,
     and a field that joins one list and not the other is neither judged nor
     swept — which does not refuse the save, it ALLOWS it, to everybody. The
     browser writes them through these same constants, so there is no fourth
     spelling to keep in step.

     The arithmetic itself is deliberately NOT here: nothing on the server
     scores anything, and a rule with no second reader is a copy waiting to
     drift the other way. */
  var PLAN_FROM = "planFrom", PLAN_TO = "planTo";
  /* Frozen and SHARED, for `hiddenSlides`' reason: every tenant with no order
     stored is handed this same array, so one careless push would give them
     all a running order. Reading never creates the field (§42). */
  var NO_FLOW = Object.freeze([]);
  function masterFlow(group) {
    var v = group && group[MASTER_FLOW];
    return Array.isArray(v) ? v : NO_FLOW;
  }
  function mayMasterPresent(w, person) {
    return isOffice(w, person);
  }

  /* ── HOW LONG EACH SUBJECT HAS (§340) ──────────────────────────────
     Islam: *"in the presentation module I need to set as an SMO the time of
     each presentation so the timer appears for the presenter"*, and then
     *"it needs to be per unit and it can be set in the master presentation
     sectoin with the flow."*

     A MAP KEYED BY TARGET, ON THE GROUP, BESIDE THE RUNNING ORDER — and it
     is deliberately NOT `units[k].extra` the way `hideSlides` above is, which
     is the nearer-looking precedent. Three reasons, in the order they decided
     it:

       A CAPABILITY IS A SUBJECT. `masterSubjects()` returns units, functions
       AND capabilities (`cap:cap6`, measured on the worked example), so a
       per-subject home needs a resolver per kind and a storage answer for the
       one kind that is not a table of its own. A map keyed by the same target
       string the flow already holds needs neither, and a subject kind added
       after today arrives working.

       THE TOTAL IS THE WHOLE REASON IT IS SET HERE. The dialog sums the
       minutes across the review — an afternoon somebody is dividing, not
       nineteen independent numbers — and a sum is one read of one map rather
       than a walk of three collections.

       AND A REFUSAL HAS TO NAME THE RIGHT SCREEN (§16.7). Stored on the unit
       this classifies as a unit setting and the refusal says Setup; stored
       here it classifies beside `masterFlow` and says the Presentation menu,
       which is where it is actually set.

     PER UNIT IS UNCHANGED BY THAT CHOICE, and it is the half that matters:
     the key is the subject, so a unit presenting ALONE — Present on its own
     page, outside any flow — reads the same number, and taking somebody out
     of the flow does not throw their minutes away.

     NOUGHT IS THE ABSENCE, DELIBERATELY. §35's rule is that absent is not
     zero, and it does not bite here because there is no such thing as a
     nought-minute slot: both mean *no time set*, which means no clock at all
     rather than a clock that starts over. `parseInt` is guarded for the same
     reason `Number("")` needed guarding at §104.10 — an empty string is 0 and
     finite, and would otherwise read as a figure somebody chose.

     Reading never creates the field (§42); the writer deletes a key put back
     to none (§50.6), so never set and set-then-cleared are the same bytes.
     THE FIELD IS NAMED ONCE and `lib/authorize.js` classifies it by this
  ...
  /* Frozen and SHARED, for `NO_FLOW`'s reason: every tenant that has set no
     ...
     all a slot. */
  var NO_MINS = Object.freeze({});
  function presentMinsMap(group) {
    var v = group && group[PRESENT_MINS];
    return (v && typeof v === "object" && !Array.isArray(v)) ? v : NO_MINS;
  }
  function presentMins(group, target) {
    var n = presentMinsMap(group)[target];
    n = typeof n === "number" ? n : parseInt(n, 10);
    return (isFinite(n) && n > 0) ? n : 0;
  }
  /* The three the office picks from (§340, Islam's own numbers, chosen over a
     list and over a typed box with the cost of each stated). NAMED ONCE and
     read by the dialog that draws the buttons AND by the check that presses
     them, so a fourth number tomorrow is one edit. */
  var PRESENT_MIN_CHOICES = Object.freeze([10, 15, 20]);

  /* ── THE CONTINGENCY FILES (spec 030) ────────────────────────────────
     Islam: *"if the platform all is down we need to have a substitue action.
     so before the review day I will download all the presnetations with a
     time stamp. so in case if things are down I have a backup to adjust to
     and present from."*

     THE WHOLE FEATURE IS THE OFFICE'S, and that is his correction of the
     first drawing, which hung a reminder on the welcome screen — a screen
     EVERY viewer opens — beside a unit head's own row. Taking these files is
     the office's act: a unit head has nothing to download, so the card is not
     drawn for them at all rather than drawn and refused (§61 avoided rather
     than met). One rule, asked by the card, by the banner and by the server
     (§42).

     THE REVIEW DAY IS NOT A MEASURING DATE, and that is Islam's own objection
     answered rather than argued with: *"the cycle is about the time we cover
     and what we report already and how we preorate more dates will create
     confusion."* He is right about the ones that are there — `from`/`to` is
     the stretch of year and `due` is when the units hand figures in — and the
     review day consults none of that arithmetic. (§307 has since taken one of
     those dates AWAY on the same objection: the month `to` is the month
     everything is measured against, and the separate `asOfMonth` is gone.) It changes no score, no
     benchmark and no proration; it says when the meeting is, so a countdown
     has something to count from. So it lives in its own block on the pen
     rather than among those three.

     AND IT COULD NOT RIDE ON `due`. That is thirteen days earlier in the
     worked example — figures in on the 15th, presented on the 28th — and it
     is free text carrying NO TIME OF DAY, so "three hours before" has nothing
     to count back from. Measured before a new field was proposed.

     Absent is off (§50.6): no review day, no reminders, ever. */
  var REMIND_HOURS = [24, 12, 6, 3];

  function mayTakeContingency(w, person) { return isOffice(w, person); }

  /* The moments, cleaned rather than trusted: a stored list is whatever a
     save carried, so it is filtered to whole positive hours, sorted widest
     first and de-duplicated — and an EMPTY stored list means "no reminders",
     which is a real answer and not a reason to fall back to the default
     (§35: absent and empty are two different things). */
  function remindHours(review) {
    var raw = review && review.remindAt;
    if (raw == null) return REMIND_HOURS.slice();
  ...
  /* The review day as a moment, or null. The DAY is what the office picks and
     the TIME is optional — a day with no time is read as 09:00 rather than
     midnight, because a review at midnight is nobody's review and the
     reminders would all have fired the evening before. */
  function reviewMoment(review) {
    var d = review && review.reviewDay;
    if (!d) return null;
    var m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(d).trim());
    if (!m) return null;
    var t = /^(\d{1,2}):(\d{2})$/.exec(String((review && review.reviewAt) || "").trim());
    var hh = t ? Number(t[1]) : 9, mm = t ? Number(t[2]) : 0;
    if (hh > 23 || mm > 59) { hh = 9; mm = 0; }
    var when = new Date(Number(m[1]), Number(m[2]) - 1, Number(m[3]), hh, mm, 0, 0);
    return isFinite(when.getTime()) ? when : null;
  }

  /* WHICH REMINDER IS DUE, as a number of hours, or null for none.
     The widest moment that has PASSED and has not been overtaken — so a
     person who opens the platform once, eight hours before, is shown the
     6-hour message and never the three earlier ones they missed. Past the
     review day itself nothing is shown: the meeting is happening, and a
     platform nagging about a backup during the review is noise. */
  function remindDue(review, now) {
    var when = reviewMoment(review);
    if (!when) return null;
    var t = (now == null ? Date.now() : Number(now));
    var left = (when.getTime() - t) / 3600000;
    if (!(left > 0)) return null;
    var hrs = remindHours(review), due = null;
    hrs.forEach(function (h) { if (left <= h && (due == null || h < due)) due = h; });
    return due;
  }

  /* Who has taken what, as an absence: `review.taken[personKey]` holds
     `{copy, slides}` timestamps and a person with neither has no entry at
     all. Read here so the card, the banner and the list cannot disagree
     about what "taken" means. */
  function contingencyTaken(review, key) {
    var all = (review && review.taken) || {};
    return all[String(key || "")] || null;
  }
  function hasBothFiles(review, key) {
    var t = contingencyTaken(review, key);
    return !!(t && t.copy && t.slides);
  }
  ...
  /* §334.18: AND THE FUNCTION'S OWN WORK IS ONE OF THEM. This read the
     capabilities alone, and §326 had just moved most of a tenant's projects
     ...
     whole feature, dead since §326), and `capProjectOf` could not say which
     ...
     was carried across at §334 and this was not; it is the same sentence, so
     it carries the same correction.

     WIDENED, NEVER NARROWED. A capability is its own destination since §334,
  ...
  /* §379 TAKES THE WORLD, because the last two lines change meaning under the
     switch: with it on, being NAMED stops being enough and being the OWNER is
     the whole test — Islam's *"contributor should not report, the owner
  ...
  /* ── MARKING YOUR OWN PROJECT FINISHED (§301) ────────────────────────
     Islam, of a project owner who could enter every figure on his project
     while the bar above them read *View only*: give him "a control to mark
     his own project finished — so the custodian can see which projects are
     ready."

     IT IS THE CONTAINER'S OWNER, AND NOT `boundedReach()`. That function
     answers "may this person report this ROW", and its contributor branch
     reaches a project through its stakeholder and collaborator lists (§147.8)
     — right for entering a figure somebody's name is against, and wrong for
     declaring the whole project finished, which is the owner's word. So the
     test is the one thing the mark means: an unbounded role speaks for the
     whole subject and may mark anything in it; a bounded role marks the
     project or pillar whose OWNER names them, and nothing else.
  ...
  /* ── WHAT DISSOLVING A BOX MOVES (§326) ─────────────────────────────────
  ...
  /* ── AND THE OTHER DIRECTION (§334, spec 048 §6) ────────────────────────
     ...
     capability starts with none, written on it afterwards (spec 048 §6). That
  ...
  return {
    ...
    focusOn: focusOn, tourReady: tourReady, unitLikeOf: unitLikeOf,
    ...
    /* §334.18: WHICH HOLDERS A SUBJECT DRAWS, exported because the authoriser
    ...
    VIDEO_MAX_BYTES: VIDEO_MAX_BYTES, VIDEO_MAX_SECS: VIDEO_MAX_SECS,
    VIDEO_PER_SUBJECT: VIDEO_PER_SUBJECT, VIDEO_STORE: VIDEO_STORE,
    VIDEO_EMBED_HOSTS: VIDEO_EMBED_HOSTS, VIDEO_SERVICES: VIDEO_SERVICES,
    videoLink: videoLink, ownHost: ownHost,
    ...
    REMIND_HOURS: REMIND_HOURS, mayTakeContingency: mayTakeContingency,
    remindHours: remindHours, reviewMoment: reviewMoment, remindDue: remindDue,
    contingencyTaken: contingencyTaken, hasBothFiles: hasBothFiles,
    greetFill: greetFill,
    ...
    CHAT_HERE_MIN: CHAT_HERE_MIN,
    chatCfg: chatCfg, chatBeat: chatBeat, chatCollectDue: chatCollectDue,
    targetParts: targetParts, unitAddedOnly: unitAddedOnly,
    kbLook: kbLook, kbAdds: kbAdds, kbAllAdds: kbAllAdds, askKey: askKey,
