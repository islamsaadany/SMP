...
(function () {
  ...
  /* THE NAME BOX, never the last .fld — a function's row ends in a SELECT that
     ...
     plans-in picker (§347.1 records the same trap in capsStep). A chip exists
     to be renamed, so landing on its name is the feature rather than a nicety.
     The Add button below still reaches for the last .fld and so still lands on
     the picker: that is the same one-line defect, raised with Islam and not yet
     answered, so it is left rather than folded into a change about something
     else (rule 1b). */
