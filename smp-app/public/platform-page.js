...
(function () {
  ...
  /* THE NAMES MOST CLIENTS HAVE, one click into the list beside them (§351).
     ...
     wrong more often than right; it works at all only because a clicked name
     lands in an ordinary editable box, so Retail becomes Retail Shops by
     typing. Islam, asked whether the lists should narrow by the industry the
     client step already collects: "not yet by industry" — one list we can
     defend beats nine we cannot, and the answer is easy to revisit once real
     clients have shown what recurs.
     The office's own function is last: every client has one by construction,
     and it is the platform's rather than the business's. */
  ...
  /* A CHIP IS HELD BY THE ROW IT MADE, NEVER BY THE SPELLING: renaming the row
     to "Retail Shops" keeps its chip off the list, and removing the row with
     the × puts the chip back — which is what a person means by "I have used
     that one". The mark is NON-ENUMERABLE on purpose: `S.shape` is posted as
     JSON and the endpoint slices the row objects through verbatim, so an
     ordinary key would travel and be stored on the client for ever. */
  ...
  function listStep(box, list, kind){
    ...
    if (S.canShape) {
      /* Above the Add button, because for most clients these ARE the answer and
         the button under them is the way to say something else. The row is not
         drawn once every name on it has been used. */
      ...
      if (left.length) {
        ...
        left.forEach(function (nm) {
          var c = el("button", "wzcm");
          c.type = "button";
          c.setAttribute("aria-label", "Add " + nm);
          c.appendChild(el("span", "p", "+"));
          c.appendChild(document.createTextNode(nm));
          c.addEventListener("click", function () {
            list.push(markFromChip(kind === "unit"
              ? { name:nm, company:"" } : { name:nm, format:"pillars" }, nm));
            S.shapeDirty = true;
            paintSetup();
            focusLastName();
          });
