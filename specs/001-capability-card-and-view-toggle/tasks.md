...
- [x] T001 [US1] `src/group-render.js`: add `capMilestonesBody(c)` and replace the
      ...
      sub-line.
- [x] T002 [US2] `src/group-render.js`: add `GVIEW` state, `viewToggle()` control,
...
- [x] T003 [US2] `src/shell.html`: wire `[data-gview]` clicks in `wire()`.
- [x] T004 `src/pageinfo.js`: extend `g_perf` Info with the toggle and the
      capability table's ranking rule.
- [x] T005 Verify: rebuild, run the QA walk (all viewers), and measure the card
