...
with it. **AMENDED 2026-09-10 at stop point C** (Islam: *"1. ok"*), and
recorded rather than overwritten: the OFFLINE COPY does not come back and
neither does the cache-by-name trap — there is no cache and no `fetch`
handler — but a worker IS served, because `push` lives in that one file and
Phase G's row says to carry notifications. The reasoning above was about
caching throughout and it stands; what it did not separate is the half that
receives a notification (§316.10).
...
- [ ] I The data — Raya carried (Islam runs it), RHI and El Abd empty, demo seeded. **PREPARED, not run** (§316.9, 2026-09-10 — the runbook is `phase-i-runbook.md`, and what Islam is asked for came out at TWO PRESSES and no SQL: **a client made by hand cannot be OPENED**, because a tenant with no graph answers 404 at the state API, while Forefront's own *Add a client* loads §67's cleared graph and deletes the row if that fails — measured both ways. `scripts/seed-demo.mjs` written (the frozen renamer AND its refusal required, never copied; refused twice, on what it builds and on what Postgres hands back; proved on a virgin database) with `checks/demo-seed.mjs` 7/7 RED both ways, scanning the COLUMNS rather than the graph and scanning Raya's as the control. **And opening an empty client found a live defect**: the landing asked the MEMBERSHIP where the state API places on the REGISTER, so a Forefront admin opening a client by rule was told *You are not on this register yet* for ever while `people` held their row — `registerKeyFor()` is the one read-only answer both use, door 44 → 49/0, `--break=membership-key` reddens exactly one. The Raya carry's route is RECORDED as stop point E asks: at deploy, from Vercel's own environment, with no second database because the frozen schemas and the shared schema are one Neon database — and its cost is a SEQUENCE, after the frozen site stops taking writes.)
- [ ] J Cutover — deployed, served, everybody signed in again, merged on Islam's word, what-to-check sent. **BUILT, not run** (§316.10, 2026-09-10 — the runbook is `phase-j-runbook.md`). **Stop point C answered by Islam** (*"1. ok"*): `/sw.js` is served again, generated from the frozen worker by `scripts/build-sw.mjs` with the notification half carried byte for byte and the caching half dropped — no `fetch` handler, nothing stored, every cache deleted on activate and the clients claimed, so no returning browser is served the frozen platform off its own disk (§91) and a tab left open on it is offered §258's reload. **The 404 every page load logged is gone**: `qa.py` at Next ERRORS none for the first time since Phase G, with `import-page` and `kb-file` green there too. **The manifest crossed with it** (`scripts/sync-static.mjs`), because on an iPhone a push is only delivered to a platform added to a home screen; the offline copy did not, which is line 47 standing. **The security headers cross** (`next.config.ts` reading the frozen `vercel.json`, §43.6): eight on every path where the new app had been serving none outside the shell's own route. **The `DELETE FROM sessions` this row asked for is not written and it is said why** — the carry copies no session, so everybody signs in once by construction (§24). **What runs at deploy is `scripts/deploy.mjs`**: the schema every build, Raya's carry and the demo seed behind one-deploy switches that never fire on a preview. Sweep: 33/229/ERRORS none, file:// none, door 49/0, state 91/0, shell 60/0 (five red runs), blob 23/23, comms 47/47, upload 9/9, demo 7/7, 588/0, 136/0, the spike 13/13, `tsc` clean. **Still Islam's**: the Vercel settings, the two switches, the address, the two presses of *Add a client* on the NEW site, and the merge.
